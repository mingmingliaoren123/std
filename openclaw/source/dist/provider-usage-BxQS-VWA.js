...
import { n as PROVIDER_LABELS, o as resolveProviderUsageDisplayName, r as clampPercent } from "./provider-usage.shared-B3BXN4xI.js";
...
function buildClaudeUsageWindows(data) {
	...
	for (const limit of data.limits ?? []) {
		if (limit.is_active === false || !Number.isFinite(limit.percent)) continue;
		...
		windows.push({
			...
			resetAt: limit.resets_at ? new Date(limit.resets_at).getTime() : void 0
		});
	}
	if (data.extra_usage?.is_enabled && Number.isFinite(data.extra_usage.utilization)) windows.push({
		label: "Extra usage",
...
async function fetchClaudeUsage(token, timeoutMs, fetchFn) {
	...
	const billing = extra?.is_enabled === true && typeof extra.used_credits === "number" && Number.isFinite(extra.used_credits) && extra.used_credits >= 0 && typeof extra.monthly_limit === "number" && Number.isFinite(extra.monthly_limit) && extra.monthly_limit >= 0 ? [{
		type: "budget",
