...
import { S as isUsageCountedSessionTranscriptFileName, T as parseUsageCountedSessionIdFromFileName, _ as isSessionArchiveArtifactName, a as resolveSessionFilePath, g as isPrimarySessionTranscriptFileName, u as resolveSessionTranscriptsDirForAgent, w as parseSessionArchiveTimestamp } from "./paths-C2C4lJH6.js";
import { r as stripInboundMetadata } from "./strip-inbound-meta-DMvJPx9F.js";
import { C as stripEnvelope, b as countToolResults, w as stripMessageIdHints, x as extractToolCallNames } from "./session-utils.fs-CR-Ydxz0.js";
import { o as normalizeUsage } from "./usage-BJjt0RVM.js";
import { a as resolveModelCostConfigFingerprint, i as resolveModelCostConfig, t as estimateUsageCost } from "./usage-format-DmMxCR4E.js";
...
async function readUsageCostCache(cachePath, pricingFingerprint) {
	try {
...
function isUsageCostCacheTempFileName(name) {
	if (!name.endsWith(".tmp") || name.startsWith(`${USAGE_COST_CACHE_FILE}.lock.`)) return false;
	return name.startsWith(".usage-cost-cache.") || name.startsWith(`${USAGE_COST_CACHE_FILE}.`);
}
async function cleanupStaleUsageCostCacheTempFiles(cachePath) {
	const dir = path.dirname(cachePath);
	const cutoffMs = Date.now() - USAGE_COST_CACHE_TEMP_FILE_GRACE_MS;
	const entries = await fs.promises.readdir(dir, { withFileTypes: true }).catch(() => []);
	await Promise.all(entries.map(async (entry) => {
		if (!entry.isFile() || !isUsageCostCacheTempFileName(entry.name)) return;
...
const formatDayKey = (date, utcOffsetMinutes) => {
	if (utcOffsetMinutes === void 0) return date.toLocaleDateString("en-CA", { timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone });
	return formatUtcDayKey(new Date(date.getTime() + utcOffsetMinutes * 60 * 1e3));
};
/**
...
*/
const fillMissingDays = (dailyMap, startMs, endMs, utcOffsetMinutes) => {
	if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || endMs < startMs) return;
	const dayMs = 1440 * 60 * 1e3;
	if (Math.floor((endMs - startMs) / dayMs) + 1 > MAX_ZERO_FILL_DAYS) return;
...
async function* readJsonlRecordsBestEffort(filePath) {
	try {
...
async function refreshCostUsageCacheForPath(params) {
	...
	try {
		...
		const cache = await readUsageCostCache(cachePath, resolveUsageCostPricingFingerprint(params?.config));
...
async function loadCostUsageSummaryFromCache(params) {
	...
	let [cache, files] = await Promise.all([readUsageCostCache(cachePath, pricingFingerprint), listUsageCountedTranscriptFiles(params.agentId)]);
	const staleFiles = getUsageCostStaleFiles({
		cache,
		files
	});
	if (params.requestRefresh !== false && staleFiles.length > 0) {
		...
		if (params.refreshMode === "sync-when-empty" && cachedFiles === 0) {
			...
			[cache, files] = await Promise.all([readUsageCostCache(cachePath, pricingFingerprint), listUsageCountedTranscriptFiles(params.agentId)]);
			if (result === "refreshed") {
...
async function loadSessionCostSummariesFromCache(params) {
	...
	const statsPromise = runTasksWithConcurrency({
		tasks: params.sessions.map((session) => async () => await fs.promises.stat(session.sessionFile).catch(() => null)),
		limit: USAGE_COST_TRANSCRIPT_STAT_CONCURRENCY
	...
	let cachedFiles = 0;
	const requiresDailyRebucket = params.dailyUtcOffsetMinutes !== void 0;
	const summaries = params.sessions.map((session, index) => {
		...
		if (summary && params.startMs !== void 0 && params.endMs !== void 0 && (requiresDailyRebucket || !isSessionSummaryContainedInRange(summary, params.startMs, params.endMs))) return entry ? buildSessionCostSummaryFromCacheEntry({
			entry,
			...
			if (entry.role === "assistant") {
				...
				if (tsLocal !== void 0) {
					...
					if (latencyMs !== void 0 && Number.isFinite(latencyMs) && latencyMs <= MAX_LATENCY_MS) {
						latencyValues.push(latencyMs);
						const dayKey = formatDayKey(entry.timestamp ?? new Date(tsLocal), params.dailyUtcOffsetMinutes);
						const dailyLatencies = dailyLatencyMap.get(dayKey) ?? [];
						dailyLatencies.push(latencyMs);
						dailyLatencyMap.set(dayKey, dailyLatencies);
					}
...
async function loadSessionLogs(params) {
	...
	const resolveCost = createUsageCostResolver(params.config);
	for await (const parsed of readJsonlRecordsBestEffort(sessionFile)) try {
		const message = parsed.message;
		if (!message) continue;
...
//#endregion
export { loadSessionCostSummary as a, resolveExistingUsageSessionFile as c, loadSessionCostSummariesFromCache as i, addCostUsageTotals as l, loadCostUsageSummary as n, loadSessionLogs as o, loadCostUsageSummaryFromCache as r, loadSessionUsageTimeSeries as s, discoverAllSessions as t, createEmptyCostUsageTotals as u };
