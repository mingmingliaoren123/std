...
import { r as sanitizeAndNormalizeEmbedding } from "./embeddings-ty_ggNrV.js";
import { C as withRemoteHttpResponse, _ as buildBatchHeaders, g as runEmbeddingBatchGroups, h as buildEmbeddingBatchGroupOptions, o as debugEmbeddingsLog, v as normalizeBatchBaseUrl } from "./memory-core-host-engine-embeddings-DVluEqwT.js";
...
async function submitGeminiBatch(params) {
	...
	const filePayload = await withRemoteHttpResponse({
		...
		onResponse: async (fileRes) => {
			...
			return readProviderJsonResponse(fileRes, "gemini.batch-file-upload");
		}
	});
	...
	return await withRemoteHttpResponse({
		...
		onResponse: async (batchRes) => {
			if (batchRes.ok) return readProviderJsonResponse(batchRes, "gemini.batch-create");
...
async function fetchGeminiBatchStatus(params) {
	...
	return await withRemoteHttpResponse({
		...
		onResponse: async (res) => {
			...
			return readProviderJsonResponse(res, "gemini.batch-status");
		}
	});
}
