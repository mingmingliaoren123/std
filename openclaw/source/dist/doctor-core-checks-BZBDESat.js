...
import { r as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-q5f2M0Ch.js";
import { t as resolveGatewayAuthToken } from "./auth-token-resolution-DX7Sy3no.js";
...
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, r as noteChromeMcpBrowserReadiness, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-DeQnPl_O.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-Ctvb4gqd.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-Rjotot4C.js";
import { i as sessionLockToRepairEffect, r as sessionLockToHealthFinding, t as detectStaleSessionLocks } from "./doctor-session-locks-DYBMzL3K.js";
import { n as disableUnavailableSkillsInConfig, r as formatMissingSkillSummary } from "./doctor-skills-core-DZQVCoNO.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-C-p212im.js";
import { n as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-Dhx0h1PR.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-BLfFhztF.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-Dw481WN4.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-CFcSTbUr.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-Dn5L6Ypi.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-D-B71ywJ.js");
...
const hooksModelCheck = {
	...
	async detect(ctx) {
		...
		const { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel } = await import("./model-selection-CpyfHlxE.js");
...
const legacyStateCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-CClpmvmZ.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-D2fHQG5B.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-CEy4_T1J.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./provider-openai-chatgpt-oauth-tls-Vx3ARZtD.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-BMCTFQHJ.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-BMCTFQHJ.js");
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-Bentjvwn.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-Bentjvwn.js");
...
const gatewayPlatformNotesCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-i7a78uSQ.js");
