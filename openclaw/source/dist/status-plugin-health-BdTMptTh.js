...
function formatCompactPluginHealthLine(snapshot) {
	...
	return parts.length === 0 ? void 0 : `⚠️ Plugins: ${parts.join(" · ")}`;
}
...
function formatDetailedPluginHealth(snapshot) {
	...
	const runtimeLoaded = runtimeLoadedIds ? new Set(runtimeLoadedIds) : void 0;
	const loaded = (runtimeLoadedIds ?? statusLoaded.map((plugin) => plugin.id)).toSorted(byLocale);
	const explainedPluginIds = new Set(snapshot.plugins.filter((plugin) => plugin.status === "error" || plugin.status === "disabled").map((plugin) => plugin.id));
	const shouldRunNotLoaded = snapshot.shouldRunPluginIds && runtimeLoaded ? snapshot.shouldRunPluginIds.filter((id) => !runtimeLoaded.has(id) && !explainedPluginIds.has(id)).toSorted(byLocale) : [];
	const shouldRunNotLoadedSet = new Set(shouldRunNotLoaded);
	const installedNotActive = runtimeLoaded ? statusLoaded.filter((plugin) => !runtimeLoaded.has(plugin.id)).map((plugin) => plugin.id).filter((id) => !shouldRunNotLoadedSet.has(id)).toSorted(byLocale) : [];
	const disabledPlugins = snapshot.plugins.filter((plugin) => plugin.status === "disabled").toSorted((left, right) => byLocale(left.id, right.id));
	...
	const unregisteredMemoryProviders = (snapshot.unregisteredMemoryEmbeddingProviders ?? []).toSorted((left, right) => byLocale(left.configuredId, right.configuredId) || byLocale(left.source, right.source));
	const lines = [
	...
	if (disabledPlugins.length > 0) {
		const disabledByReason = /* @__PURE__ */ new Map();
		for (const plugin of disabledPlugins) {
			const reason = plugin.error ?? "disabled";
			const ids = disabledByReason.get(reason);
			if (ids) ids.push(plugin.id);
			else disabledByReason.set(reason, [plugin.id]);
		}
		const reasonEntries = [...disabledByReason.entries()].toSorted((left, right) => byLocale(left[0], right[0]));
		lines.push(...reasonEntries.slice(0, 8).map(([reason, ids]) => `- ${reason}: ${ids.length} (${formatPluginList(ids, 8)})`));
		if (reasonEntries.length > 8) lines.push(`- +${reasonEntries.length - 8} more reasons`);
	}
	if (installedNotActive.length > 0) lines.push(`Installed (not active): ${installedNotActive.length} (${formatPluginList(installedNotActive, 8)})`);
	if (shouldRunNotLoaded.length > 0) lines.push(`Configured to run but not loaded: ${shouldRunNotLoaded.length} (${formatPluginList(shouldRunNotLoaded, 8)})`);
	if (unregisteredMemoryProviders.length > 0) {
		const display = unregisteredMemoryProviders.map((entry) => `${entry.configuredId} (memorySearch.${entry.source})`);
		lines.push(`Configured memory provider not registered: ${unregisteredMemoryProviders.length} (${formatPluginList(display, 8)})`);
	}
