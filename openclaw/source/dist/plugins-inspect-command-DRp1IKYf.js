...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, buildPluginDiagnosticsReport, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-1zl9Bjo_.js");
	const { loadInstalledPluginIndexInstallRecords } = await import("./installed-plugin-index-records-CTA0CdYc.js");
	...
	if (!targetPlugin) {
		if (id === "skill-workshop") {
			const { detectSkillWorkshopToolPolicyDiagnostic } = await import("./tool-policy-diagnostic-BTMmza6H.js");
