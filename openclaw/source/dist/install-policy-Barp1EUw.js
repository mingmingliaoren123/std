...
import { i as normalizePositiveTimerMs, r as normalizePositiveInt } from "./shared-BV5NPfdY.js";
...
async function runPolicyCommand(params) {
	return await new Promise((resolve, reject) => {
		...
		const failCommand = (error, kill) => {
			if (settled) return;
			settled = true;
			clearTimers();
			if (kill) forceKillChildProcessTree(child);
		...
		const append = (chunk, target) => {
			...
			if (outputBytes > params.maxOutputBytes) {
				failCommand(/* @__PURE__ */ new Error(`output exceeded maxOutputBytes (${params.maxOutputBytes})`), true);
				return;
			}
		...
		child.stdout?.on("error", (error) => {
			failCommand(/* @__PURE__ */ new Error(`policy stdout stream failed: ${formatErrorMessage(error)}`), true);
		});
		child.stdout?.on("data", (chunk) => append(chunk, "stdout"));
		child.stderr?.on("error", (error) => {
			failCommand(/* @__PURE__ */ new Error(`policy stderr stream failed: ${formatErrorMessage(error)}`), true);
		});
		...
		const handleStdinError = (error) => {
			...
			failCommand(/* @__PURE__ */ new Error(`policy stdin stream failed: ${formatErrorMessage(error)}`), true);
		};
...
async function runInstallPolicy(params) {
	...
	if (!config) try {
		const { getRuntimeConfig } = await import("./io-CXugYKHV.js");
