	...
	for (const block of presentation.blocks) {
		...
		if (block.type === "buttons") {
			const labels = block.buttons.map((button) => {
				...
				if (targetUrl) return `${button.label}: ${targetUrl}`;
				const controlValue = button.action?.type === "command" ? resolveMessagePresentationControlValue(button) : void 0;
				if (controlValue && !button.disabled) return `${button.label}: \`${controlValue}\``;
				return button.label;
			}).filter(Boolean);
