...
async function readResponseBodySnippet(response, limits) {
	try {
		...
		if (!body || typeof body.getReader !== "function") {
			...
			if (encoded.byteLength > limits.maxBytes) return truncateUtf16Safe(new TextDecoder().decode(encoded.subarray(0, limits.maxBytes), { stream: true }), limits.maxChars);
		...
		return truncateUtf16Safe(new TextDecoder().decode(Buffer.concat(chunks, total)), limits.maxChars);
	} catch {
		return "";
	}
}
