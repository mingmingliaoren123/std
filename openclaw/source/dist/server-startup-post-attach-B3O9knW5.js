...
import { i as getPluginModuleLoaderStats } from "./plugin-module-loader-cache-uqaaAPup.js";
import { t as STARTUP_UNAVAILABLE_GATEWAY_METHODS } from "./core-descriptors-DRUtdasO.js";
import { o as hasRestartSentinel } from "./restart-sentinel-CEN_sCi1.js";
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-TYP8gbsd.js"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-RurGC76M.js"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-CpyfHlxE.js"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-CHGiuwpC.js"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-cj6qAdAU.js"));
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-C31TuSvD.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-DnAUi_-1.js");
...
function scheduleAgentRuntimePluginPrewarm(params) {
	...
	timer = setTimeout(() => {
		...
		measureStartup(params.startupTrace, "post-ready.agent-runtime-plugins", async () => {
			...
			const { ensureRuntimePluginsLoaded } = await import("./runtime-plugins-CCOHat8D.js");
			const cfg = params.getConfig();
			if (isStopped()) return;
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-BHgBzwuS.js");
			if (isStopped()) return;
...
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-BFjeD3FX.js");
...
async function prewarmConfiguredPrimaryModel(params) {
	...
	const [{ resolveAgentWorkspaceDir, resolveDefaultAgentDir, resolveDefaultAgentId }, { DEFAULT_MODEL, DEFAULT_PROVIDER }, { isCliProvider, resolveConfiguredModelRef }] = await Promise.all([
		import("./agent-scope-w9VdIGfQ.js"),
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-BxX0hSc8.js");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([loadInternalHooksModule(), import("./loader-9BITR95c.js")]);
	...
	let pluginServices = params.shouldStartPluginServices?.() === false ? null : await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-CIGmZCbX.js");
	...
	schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			try {
				const [{ resolveAgentSessionDirs }, { cleanStaleLockFiles }] = await Promise.all([import("./session-dirs-BUmx3Yg2.js"), import("./session-write-lock-DFzZA2AG.js")]);
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-B93EmgUG.js");
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-NyjE4tvi.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				loadAgentDefaultsModule(),
				import("./model-catalog-BAFTl1f7.js"),
				loadAgentModelSelectionModule()
			]);
			if (isStopped()) return;
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-mxipLyo5.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-DaRHJn11.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-C7WGeJof.js")).startGatewayTailscaleExposure(...args)
};
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	sidecarsPromise.then(async (sidecarsResult) => {
		...
		if (hookRunner?.hasHooks("gateway_start")) {
			const { withPluginHttpRouteRegistry } = await import("./http-registry-B58Ar4V7.js");
