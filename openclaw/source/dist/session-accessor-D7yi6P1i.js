...
import { a as redactSensitiveFieldValue, c as redactSensitiveText, g as readLoggingConfig, i as redactSecrets, n as getDefaultRedactPatterns } from "./redact-B9QQ4Wyz.js";
...
import { $ as parseSessionThreadInfo, L as resolveSessionStorePathForScope, P as resolveSessionStoreEntry, S as loadSessionStore, _ as updateLastRoute, a as cleanupSessionLifecycleArtifacts$1, at as normalizeSessionEntrySlotKey, c as listSessionEntries$1, ct as resolveSessionStoreAgentId, d as projectSessionEntryForPersistenceRevision, f as purgeDeletedAgentSessionEntries$1, h as resetSessionEntryLifecycle$1, l as patchSessionEntry$1, lt as resolveSessionStoreKey, m as recordSessionMetaFromInbound, n as applySessionEntryPatchProjection, o as deleteSessionEntryLifecycle$1, p as readSessionUpdatedAt$1, s as getSessionEntry, t as applySessionEntryLifecycleMutation$1, u as patchSessionEntryWithKey$1, v as updateSessionStore, y as updateSessionStoreEntry } from "./store-BJJhlPrk.js";
import { a as resolveSessionFilePath, c as resolveSessionTranscriptPathInDir, d as resolveStorePath, m as formatSessionArchiveTimestamp, o as resolveSessionFilePathOptions, s as resolveSessionTranscriptPath } from "./paths-C2C4lJH6.js";
...
import { a as withOwnedSessionTranscriptWrites, c as appendSerializedJsonlEntry, d as serializeJsonlLine, i as resolveOwnedSessionTranscriptWriteLockRunner, m as writeJsonlLines, o as appendJsonlEntry, p as writeJsonlEntry, u as serializeJsonlEntry } from "./transcript-write-context-CoTZvex2.js";
...
const loadSessionForkTranscriptRuntime = createLazyRuntimeModule(() => import("./session-fork-transcript.runtime.js"));
...
function loadSessionEntry(scope) {
	if (scope.clone === false || scope.readConsistency === "latest") return resolveSessionStoreEntry({
		store: loadSessionStore(resolveSessionStorePathForScope(scope), {
...
function openSessionEntryReadView(scope = {}) {
	const store = loadSessionStore(resolveSessionStorePathForScope({
		...scope,
		sessionKey: ""
	}), {
		clone: false,
	...
	return {
		get: (sessionKey) => Object.hasOwn(store, sessionKey) ? store[sessionKey] : void 0,
...
*/
async function createForkedSessionTranscript(params) {
	...
	const entries = params.buildEntries?.({
		sessionId,
		timestamp
	}) ?? [];
	fs.mkdirSync(path.dirname(sessionFile), { recursive: true });
...
async function forkSessionFromParentTranscript(params) {
	...
	try {
		parentSessionFile = resolveSessionFilePath(params.parentEntry.sessionId, params.parentEntry, resolveSessionFilePathOptions({
	...
	if (!parentSessionFile) return { status: "missing-parent" };
	try {
		const { buildForkedBranchEntries, forkSourceHasAssistantEntry, readForkSourceTranscript } = await loadSessionForkTranscriptRuntime();
		const source = await readForkSourceTranscript(parentSessionFile);
		if (!source) return { status: "failed" };
		const shouldPersistBranch = source.preserveLeafControl || forkSourceHasAssistantEntry(source.branchEntries);
		const targetSessionsDir = params.targetStorePath ? path.dirname(params.targetStorePath) : source.sessionDir;
		return {
			status: "created",
			transcript: shouldPersistBranch ? await createForkedSessionTranscript({
				cwd: source.cwd,
				parentSessionFile,
				sessionsDir: targetSessionsDir,
				buildEntries: ({ timestamp }) => buildForkedBranchEntries({
					source,
					timestamp
				})
...
async function forkSessionEntryFromParentTarget(params) {
	return await updateSessionStore(params.storePath, async (store) => {
		...
		const entry = resolveEntryFromStoreKeys({
			store,
			keys: params.sessionTarget.storeKeys
		...
		if (forked.status !== "created") return { status: "failed" };
...
async function recordInboundSessionMeta(params) {
	return await recordSessionMetaFromInbound(params);
}
/**
...
async function updateSessionLastRoute(params) {
	return await updateLastRoute(params);
}
...
async function commitReplySessionInitialization(params) {
	const committed = await updateSessionStore(params.storePath, async (store) => {
		...
		store[resolved.normalizedKey] = currentEntry ? mergeConcurrentReplySessionMetadata({
			currentEntry,
...
async function persistExpectedSessionTranscriptTurn(scope, options) {
	...
	const updated = await updateSessionEntry({
		...
		if (currentEntry.sessionId !== expectedSessionId || options.expectedLifecycleRevision !== void 0 && currentEntry.lifecycleRevision !== options.expectedLifecycleRevision) {
	...
	if (rejectedEntry || updated?.sessionId !== expectedSessionId || options.expectedLifecycleRevision !== void 0 && updated.lifecycleRevision !== options.expectedLifecycleRevision) return {
		appendedCount: 0,
		messages: [],
...
//#endregion
export { OPENCLAW_DELIVERY_MIRROR_MODEL as $, purgeDeletedAgentSessionEntries as A, resolveSessionTranscriptRuntimeTarget as B, patchSessionEntry as C, preflightSessionTranscriptForManualCompact as D, persistSessionTranscriptTurn as E, resolveSessionAbortTarget as F, updateSessionLastRoute as G, trimSessionTranscriptForManualCompact as H, resolveSessionEntryAccessTarget as I, appendSessionTranscriptMessage as J, upsertSessionEntry as K, resolveSessionEntryCandidateTarget as L, recordInboundSessionMeta as M, replaceSessionEntry as N, preserveTemporarySessionMapping as O, resetSessionEntryLifecycle as P, readFileRangeAsync as Q, resolveSessionTranscriptReadTarget as R, openSessionEntryReadView as S, persistSessionResetLifecycle as T, updateResolvedSessionEntry as U, restoreSessionFromCompactionCheckpoint as V, updateSessionEntry as W, streamSessionTranscriptLines as X, runSessionTranscriptAppendTransaction as Y, streamSessionTranscriptLinesReverse as Z, listSessionEntries as _, applySessionPatchProjection as a, redactTranscriptMessage as at, loadTranscriptEvents as b, cleanupPluginHostSessionStore as c, createForkedSessionTranscript as d, OPENCLAW_TRANSCRIPT_ARTIFACT_API as et, createSessionEntryWithTranscript as f, forkSessionFromParentTranscript as g, forkSessionEntryFromParentTarget as h, applySessionEntryLifecycleMutation as i, isTranscriptOnlyOpenClawAssistantModel as it, readSessionUpdatedAt as j, publishTranscriptUpdate as k, cleanupSessionLifecycleArtifacts as l, findTranscriptEvent as m, appendTranscriptMessage as n, isOpenClawDeliveryMirrorAssistantMessage as nt, branchSessionFromCompactionCheckpoint as o, resolveAndPersistSessionFile as ot, deleteSessionEntryLifecycle as p, resolveSessionTranscriptFile as q, applyRestartRecoveryLifecycle as r, isTranscriptOnlyOpenClawAssistantMessage as rt, canonicalizeSessionEntryAliases as s, clearPluginOwnedSessionState as st, appendTranscriptEvent as t, OPENCLAW_TRANSCRIPT_ARTIFACT_PROVIDER as tt, commitReplySessionInitialization as u, loadReplySessionInitializationSnapshot as v, patchSessionEntryWithKey as w, markSessionAbortTarget as x, loadSessionEntry as y, resolveSessionTranscriptRuntimeReadTarget as z };
