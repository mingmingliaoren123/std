...
function listenForIMessageCliStreamErrors(params) {
	for (const stream of ["stdout", "stderr"]) params.child[stream].on("error", (error) => {
		if (params.isSettled()) return;
		params.fail(/* @__PURE__ */ new Error(`iMessage CLI ${stream} stream error: ${error.message}`));
		try {
...
//#endregion
export { listenForIMessageCliStreamErrors as i, appendIMessageCliStderrTail as n, appendIMessageCliStdout as r, extractMarkdownFormatRuns as t };
