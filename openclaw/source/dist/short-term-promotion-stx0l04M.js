...
import { H as formatMemoryDreamingDay, U as isSameMemoryDreamingDay } from "./dreaming-DqzMSXGC.js";
...
function isContaminatedDreamingSnippet(raw) {
	...
	if (/<!--\s*openclaw-memory-promotion:/i.test(snippet) || DREAMING_TRANSCRIPT_PROMPT_LINE_RE.test(snippet) || RAW_SESSION_METADATA_RE.test(snippet) || RAW_CONVERSATION_SUMMARY_RE.test(snippet) || RAW_TRANSCRIPT_TURN_RE.test(snippet) || MEMORY_FLUSH_PROMPT_RE.test(snippet) || PROMOTION_SCORE_METADATA_RE.test(snippet)) return true;
...
async function writeRawShortTermStoreForTest(workspaceDir, raw, namespace, metaKey) {
	const record = asNullableRecord(raw);
...
const testing = {
	...
	writeRawRecallStore: (workspaceDir, raw) => writeRawShortTermStoreForTest(workspaceDir, raw, SHORT_TERM_RECALL_NAMESPACE, "recall"),
	writeRawPhaseSignalStore: (workspaceDir, raw) => writeRawShortTermStoreForTest(workspaceDir, raw, SHORT_TERM_PHASE_SIGNAL_NAMESPACE, "phase"),
...
//#endregion
export { resolveShortTermRecallLockPath as C, repairShortTermPromotionArtifacts as S, testing as T, recordDreamingPhaseSignals as _, SHORT_TERM_STORE_RELATIVE_PATH as a, recordShortTermRecalls as b, filterFreshLightDreamingEntries as c, loadShortTermPromotionDreamingStats as d, normalizeShortTermPhaseSignalStore as f, readShortTermRecallEntries as g, readLightStagedKeys as h, SHORT_TERM_PHASE_SIGNAL_RELATIVE_PATH as i, filterLiveShortTermRecallEntries as l, rankShortTermPromotionCandidates as m, DEFAULT_PROMOTION_MIN_SCORE as n, applyShortTermPromotions as o, normalizeShortTermRecallStore as p, DEFAULT_PROMOTION_MIN_UNIQUE_QUERIES as r, auditShortTermPromotionArtifacts as s, DEFAULT_PROMOTION_MIN_RECALL_COUNT as t, isShortTermMemoryPath as u, recordGroundedShortTermCandidates as v, resolveShortTermRecallStorePath as w, removeGroundedShortTermCandidates as x, recordRemConsideredPhaseSignals as y };
