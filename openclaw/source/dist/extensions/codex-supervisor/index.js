...
import { t as buildJsonPluginConfigSchema } from "../../config-schema-ByzWLagI.js";
...
var WebSocketCodexJsonRpcConnection = class extends BaseCodexJsonRpcConnection {
	constructor(endpoint) {
		...
		this.ws = endpoint.url.startsWith("unix://") ? new WebSocket$1("ws://localhost/", {
			headers,
			createConnection: () => connectCodexSupervisorUnixSocket(endpoint.url)
		}) : new WebSocket$1(endpoint.url, { headers });
	...
	async close() {
		...
		if (this.ws.readyState === WebSocket$1.CLOSED) return;
		await new Promise((resolve) => {
			...
			if (this.ws.readyState === WebSocket$1.CONNECTING || this.ws.readyState === WebSocket$1.OPEN) this.ws.close();
			else {
				clearTimeout(timeout);
				resolve();
			}
