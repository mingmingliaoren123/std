...
function assertAllowedTrustedHostnameResolvedAddressesOrThrow(results, hostname) {
	...
	for (const entry of results) if (!isLoopbackAllowed && isLoopbackIpAddressIncludingEmbeddedIpv4(entry.address) || isLinkLocalIpAddress(entry.address) || isCloudMetadataIpAddress(entry.address)) throw new SsrFBlockedError(BLOCKED_RESOLVED_IP_MESSAGE);
}
