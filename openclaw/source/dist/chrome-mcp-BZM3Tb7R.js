...
import { C as BrowserTabNotFoundError, b as BrowserProfileUnavailableError } from "./cdp.helpers-B9jipAGQ.js";
import "./tmp-openclaw-dir-DRHs_iAK.js";
import "./record-shared-BXFJBdq0.js";
...
async function callTargetTool(params, name, args) {
	return await callTool(params.profileName, chromeMcpProfileOptionsFromParams(params), name, args, params);
}
...
async function findPageById(profileName, pageId, profileOptions, options = {}) {
	const page = (await listChromeMcpPages(profileName, profileOptions, options)).find((entry) => entry.id === pageId);
...
async function closeChromeMcpTab(profileName, targetId, profileOptions, options = {}) {
	await callTool(profileName, profileOptions, "close_page", { pageId: parsePageId(targetId) }, options);
}
...
async function navigateChromeMcpPage(params) {
	...
	return { url: (await findPageById(params.profileName, parsePageId(params.targetId), chromeMcpProfileOptionsFromParams(params), {
		timeoutMs: callTimeoutMs,
		signal: params.signal
	})).url ?? params.url };
}
...
async function takeChromeMcpSnapshot(params) {
	return extractSnapshot(await callTargetTool(params, "take_snapshot", { pageId: parsePageId(params.targetId) }));
}
...
async function evaluateChromeMcpScript(params) {
	return extractJsonMessage(await callTargetTool(params, "evaluate_script", {
		pageId: parsePageId(params.targetId),
		function: params.fn,
...
//#endregion
export { takeChromeMcpScreenshot as C, setChromeMcpSessionFactoryForTest as S, uploadChromeMcpFile as T, pressChromeMcpKey as _, decodeChromeMcpStderrTail as a, resolveChromeMcpNavigateCallTimeoutMs as b, evaluateChromeMcpScript as c, focusChromeMcpTab as d, getChromeMcpPid as f, openChromeMcpTab as g, navigateChromeMcpPage as h, closeChromeMcpTab as i, fillChromeMcpElement as l, listChromeMcpTabs as m, clickChromeMcpElement as n, dragChromeMcpElement as o, hoverChromeMcpElement as p, closeChromeMcpSession as r, ensureChromeMcpAvailable as s, clickChromeMcpCoords as t, fillChromeMcpForm as u, resetChromeMcpSessionsForTest as v, takeChromeMcpSnapshot as w, setChromeMcpProcessCleanupDepsForTest as x, resizeChromeMcpPage as y };
