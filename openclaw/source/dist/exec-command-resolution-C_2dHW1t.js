...
import { C as resolveDispatchWrapperTrustPlan, T as unwrapKnownDispatchWrapperInvocation, c as isShellWrapperExecutable, d as unwrapKnownShellMultiplexerInvocation, r as extractBindableShellWrapperInlineCommand } from "./shell-wrapper-resolution-Cy6xKD1y.js";
...
function resolveWindowsExecutableExtensions(executable, env) {
	...
	return ["", ...(env?.PATHEXT ?? env?.PathExt ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.PathExt ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => normalizeLowercaseStringOrEmpty(ext))];
}
function resolveWindowsExecutableExtSet(env) {
	return new Set((env?.PATHEXT ?? env?.PathExt ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.PathExt ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => normalizeLowercaseStringOrEmpty(ext)).filter(Boolean));
}
...
function resolveExecutableFromPathEnv(executable, pathEnv, env) {
	...
	for (const entry of entries) for (const ext of extensions) {
		...
		if (isExecutableFile(candidate, { env })) return candidate;
	}
}
...
const KNOWN_PATHEXT = /* @__PURE__ */ new Set([
	".com",
	".exe",
	".bat",
	".cmd"
