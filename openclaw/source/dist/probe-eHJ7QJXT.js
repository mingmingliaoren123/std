...
import { u as isPrivateIpAddress } from "./ssrf-BayeDjCv.js";
...
import { n as loadOutboundMediaFromUrl } from "./outbound-media-DYSQY8nu.js";
import "./ssrf-policy-DKsCBaFj.js";
...
import { C as describeBotFrameworkServiceUrlHost, D as resolveMSTeamsSdkCloudOptions, O as validateMSTeamsProactiveServiceUrlBoundary, T as normalizeBotFrameworkServiceUrl, _ as readAccessToken, b as loadMSTeamsSdkWithAuth, h as resolveMSTeamsCredentials, m as loadDelegatedTokens, w as isAllowedBotFrameworkServiceUrl, x as buildUserAgent, y as createMSTeamsTokenProvider } from "./graph-users-Caq3xHbh.js";
import { c as createMSTeamsHttpError } from "./oauth.token-DnwthC4R.js";
import { i as resolveMSTeamsRouteConfig, r as resolveMSTeamsReplyPolicy } from "./policy-BHrcbPLi.js";
import { i as isRevokedProxyError, n as formatMSTeamsSendErrorHint, r as formatUnknownError, t as classifyMSTeamsSendError } from "./errors-BUijkII6.js";
import { C as toPluginJsonValue, S as resolveMSTeamsSqliteStateEnv, o as buildMSTeamsPollCard, v as createMSTeamsConversationStoreState, w as withMSTeamsSqliteMutationLock } from "./polls-DYht7mPK.js";
...
async function uploadToOneDrive(params) {
	...
	const data = await readProviderJsonResponse(res, "msteams.graph-upload.uploadOneDriveFile");
...
async function createSharingLink(params) {
	...
	const data = await readProviderJsonResponse(res, "msteams.graph-upload.createOneDriveSharingLink");
...
async function uploadToSharePoint(params) {
	...
	const data = await readProviderJsonResponse(res, "msteams.graph-upload.uploadSharePointFile");
...
async function getDriveItemProperties(params) {
	...
	const data = await readProviderJsonResponse(res, "msteams.graph-upload.getDriveItemProperties");
...
async function resolveGraphChatId(params) {
	...
	const chats = (await readProviderJsonResponse(res, "msteams.graph-upload.getOneOnOneChatId")).value ?? [];
...
async function getChatMembers(params) {
	...
	return ((await readProviderJsonResponse(res, "msteams.graph-upload.getChatMembers")).value ?? []).map((m) => ({
...
async function createSharePointSharingLink(params) {
	...
	const data = await readProviderJsonResponse(res, "msteams.graph-upload.createSharePointSharingLink");
...
const loadMSTeamsApiModule = createLazyRuntimeModule(() => import("./dist-C9eZrdKb.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)));
