...
const loadRemoteSkillsRuntimeModule = async () => await import("./remote-zVeM1Y-k.js");
...
async function startGatewayPluginDiscovery(params) {
	...
	return await measureStartup(params.startupTrace, "runtime.early.discovery.start", async () => {
		const { startGatewayDiscovery } = await import("./server-discovery-runtime-BNDlcT2Q.js");
...
async function startGatewayEarlyRuntime(params) {
	...
	if (!params.minimalTestGateway) {
		import("./context-O1Clq4JF.js").then(({ ensureContextWindowCacheLoaded }) => ensureContextWindowCacheLoaded(params.cfgAtStart)).catch((err) => {
			params.log.warn(`Context-window cache warmup failed to start: ${String(err)}`);
		});
		const [{ primeRemoteSkillsCache, setSkillsRemoteRegistry }, taskRegistryMaintenance] = await measureStartup(params.startupTrace, "runtime.early.lazy-runtime-imports", () => Promise.all([loadRemoteSkillsRuntimeModule(), import("./task-registry.maintenance-mSclxKui.js")]));
	...
	const skillsChangeUnsub = params.minimalTestGateway ? () => {} : await measureStartup(params.startupTrace, "runtime.early.skills-listener", async () => {
		const [{ registerSkillsChangeListener }, { refreshRemoteBinsForConnectedNodes }] = await Promise.all([import("./refresh-BNozQvNp.js"), loadRemoteSkillsRuntimeModule()]);
	...
	const startMaintenance = async () => {
		...
		return await measureStartup(params.startupTrace, "post-ready.maintenance", async () => {
			const { startGatewayMaintenanceTimers } = await import("./server-maintenance-DZtSf_TM.js");
