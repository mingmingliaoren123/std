...
import { a as emitInternalDiagnosticEvent } from "./diagnostic-events-JZsXee1S.js";
...
import { i as copyReplyPayloadMetadata } from "./reply-payload-BK_jICQ3.js";
...
import { t as diagnosticErrorCategory } from "./diagnostic-error-metadata-yrw7ASmg.js";
import { C as isPreConnectNetworkError, T as OutboundDeliveryError, _ as attachOutboundDeliveryCommitHook, a as withActiveDeliveryClaim, c as failDelivery, h as markDeliveryPlatformSendDispatched, l as failDeliveryAfterPlatformSend, m as markDeliveryPlatformSendAttemptStarted, o as ackDelivery, p as markDeliveryPlatformOutcomeUnknown, s as enqueueDelivery, u as failDeliveryBeforePlatformSend, v as runOutboundDeliveryCommitHooks } from "./delivery-queue-Dwxuz2Ms.js";
import { a as hasReplyPayloadContent, f as renderMessagePresentationFallbackText, l as normalizeMessagePresentation } from "./payload-C0zJd_02.js";
import { c as resolveTextChunkLimit, i as chunkMarkdownTextWithMode, n as chunkByParagraph, s as resolveChunkMode } from "./chunk-eqDomQ-g.js";
import { a as unknownSendReconciliationKinds } from "./types-GcWljJIT.js";
import { n as adaptMessagePresentationForChannel } from "./interactive-BgW8Fssg.js";
...
import { C as createReplyToDeliveryPolicy } from "./reply-payload-C3J477-H.js";
import { l as summarizeOutboundPayloadForTransport, t as createOutboundPayloadPlan } from "./payloads-DYHv5qoG.js";
...
async function deliverOutboundPayloadsInternal(params) {
	...
	if (params.requireUnknownSendReconciliation === true && payloads.length !== 1) throw new Error(`Required durable message send is unsupported for ${channel}: unknown-send reconciliation requires exactly one payload`);
...
async function deliverOutboundPayloadsWithQueueCleanup(params, queueId) {
	...
	try {
		...
		if (queueId) if (hadPartialFailure) {
			...
			if (postSendState === void 0 || postSendState === "marked") await (!partialSendEvidence && partialFailuresArePreConnect ? failDeliveryBeforePlatformSend : failDelivery)(queueId, error).catch((err) => {
			...
			else if (postSendState === "acked") await runCommitHooksAfterAck();
		} else {
		...
		if (queueId) {
			if (isDeliveryAbortError(err)) await ackDelivery(queueId).catch(() => {});
				...
				await runCommitHooksAfterAck();
			} else await (isPreConnectNetworkError(err) ? failDeliveryBeforePlatformSend : failDelivery)(queueId, formatErrorMessage(err)).catch((failErr) => {
		...
		try {
			...
			for (const delivery of deliveries) {
				...
				for (const receiptId of receiptIds) {
					const matchingIndices = reportedResults.filter((reported) => availableReportedIndices.has(reported.resultIndex) && !coveredIndices.includes(reported.resultIndex) && results[reported.resultIndex]?.channel === delivery.channel && resultPlatformIds(results[reported.resultIndex]).has(receiptId)).map((reported) => reported.resultIndex);
