...
import { o as inspectPortUsage } from "./ports-3JAu4m81.js";
import { a as isGatewayArgv } from "./windows-port-pids-jYst3qTE.js";
...
import { t as findVerifiedGatewayListenerPidsOnPortSync } from "./gateway-processes-ZnwhPcAT.js";
import { t as execSchtasks } from "./schtasks-exec-CE-u8bHT.js";
...
function findInstalledProcessPid(entries, port, installedArguments, matchesProcess) {
	for (const entry of entries) {
		...
		if (!matchesProcess(argv) || parsePortFromProgramArguments(argv) !== port || !matchesInstalledProgramArguments(argv, installedArguments)) continue;
...
async function resolveFallbackRuntime(env, installedCommand, mode = "observe") {
	...
	const port = parsePortFromProgramArguments(command?.programArguments) ?? parsePositivePort(command?.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(env);
	if (!port) return {
		status: "unknown",
...
async function assertReplacementPortAvailableForTakeover(params) {
	...
	const port = parsePortFromProgramArguments(params.programArguments) ?? parsePositivePort(params.environment?.OPENCLAW_GATEWAY_PORT) ?? resolveConfiguredGatewayPort(params.env);
...
//#endregion
export { readWindowsStartupFallbackRuntimeForUpdate as a, resumeScheduledTaskAutoStartAfterUpdate as c, suspendScheduledTaskAutoStartForUpdate as d, uninstallScheduledTask as f, readScheduledTaskRuntime as i, stageScheduledTask as l, isScheduledTaskInstalled as n, resolveTaskScriptPath as o, readScheduledTaskCommand as r, restartScheduledTask as s, installScheduledTask as t, stopScheduledTask as u };
