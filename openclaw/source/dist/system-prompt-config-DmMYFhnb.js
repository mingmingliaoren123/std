...
import { Q as resolveOwnerDisplaySetting } from "./io-By0s-a_s.js";
import { k as isOpenClawMainPromptSurface } from "./command-registration-tKF3dsKu.js";
import { t as buildMemoryPromptSection } from "./memory-state-DefveORB.js";
import { t as normalizeChatType } from "./chat-type-BARlA53h.js";
import { r as listDeliverableMessageChannels } from "./message-channel-normalize-s5PUEeqJ.js";
import "./message-channel-CB9y2CYk.js";
import { n as SILENT_REPLY_TOKEN } from "./tokens-DKI4eGAu.js";
import { i as isKnownNativeApprovalPromptChannel, r as hasNativeApprovalPromptRuntimeCapability } from "./native-approval-prompt-DjKtlncz.js";
import { l as resolveEffectiveToolFsWorkspaceOnly } from "./local-roots-CAoJyC6u.js";
import { t as sanitizeForPromptLiteral } from "./sanitize-for-prompt-DSJzLJDI.js";
import { n as buildTtsSystemPromptHint } from "./tts-runtime-DGW70YBV.js";
...
function buildMessagingSection(params) {
	...
	return [
		...
		params.availableTools.has("message") ? [
			...
			groupMessageToolOnly ? "- Group/channel etiquette: for stale threads, jokes, lightweight acknowledgements, or low-value chatter, prefer a reaction when available or no channel message; when a visible reply is warranted, use `message(action=send)` because final text stays private." : "",
			...
			messageToolOnly ? "- If you use `message` (`action=send`) to deliver visible output, do not repeat that visible content in your final answer." : suppressSilentTokenGuidance ? "- Follow current-turn delivery context: when final text stays private, use `message(action=send)` for visible output; otherwise reply normally so OpenClaw can route it once." : `- If you use \`message\` (\`action=send\`) to deliver your user-visible reply, respond with ONLY: ${SILENT_REPLY_TOKEN} (avoid duplicate replies).`,
