...
import { f as normalizeOptionalAgentId, u as normalizeAgentId } from "./session-key-VWT_xzM9.js";
import { H as resolveCronTriggerMinIntervalMs } from "./io-By0s-a_s.js";
...
import { r as stripTargetProviderPrefix } from "./channel-target-prefix-CJ1Q19f6.js";
import { t as resolveExplicitDeliveryTargetCompat } from "./target-parsing-loaded-pi4a9cP7.js";
...
import { c as normalizePayloadToSystemText, l as normalizeRequiredName, t as assertSafeCronSessionTargetId, u as coerceFiniteScheduleNumber } from "./session-target-YpyFax1g.js";
...
import { n as resolveOutboundTargetWithPlugin, t as resolveSessionDeliveryTarget } from "./targets-session-Bj3WupqQ.js";
...
import { n as computePreviousRunAtMs, t as computeNextRunAtMs } from "./schedule-DFQuHnaZ.js";
...
function isStaggeredCronRunAtMs(job, runAtMs) {
	...
	return computeStaggeredCronPreviousRunAtMs(job, runAtMs + 1e3) === runAtMs;
}
...
function recomputeNextRunsForMaintenance(state, opts) {
	...
	if (state.store && deferralIds.size > 0) {
		const relevantDeferralIds = new Set(state.store.jobs.filter((job) => isJobEnabled(job)).map((job) => job.id));
		for (const jobId of deferralIds) if (!relevantDeferralIds.has(jobId)) deferralIds.delete(jobId);
	}
	return walkSchedulableJobs(state, ({ job, nowMs: now }) => {
		let changed = false;
		...
		} else if (repairFutureCronNextRunAtMs && !deferralIds.has(job.id) && shouldRepairFutureCronNextRunAtMs({
			state,
			job,
			nowMs: now
		})) {
...
function applyAgentTurnToolsAllowPatch(payload, patch, existing) {
	if (Array.isArray(patch.toolsAllow)) {
		...
		if (existing ? existing.toolsAllowIsDefault === true && toolsAllowEqual(existing, patch) : patch.toolsAllowIsDefault === true) payload.toolsAllowIsDefault = true;
...
//#endregion
export { resolveJobErrorBackoffUntilMs as _, computeJobNextRunAtMs as a, resolveDeliveryTarget as b, errorBackoffMs as c, isJobDue as d, isJobEnabled as f, recordScheduleComputeError as g, recomputeNextRunsForMaintenance as h, assertSupportedJobSpec as i, findJobOrThrow as l, recomputeNextRuns as m, applyDeclarativeJobSpec as n, computeJobPreviousRunAtMs as o, nextWakeAtMs as p, applyJobPatch as r, createJob as s, DEFAULT_ERROR_BACKOFF_SCHEDULE_MS as t, hasScheduledNextRunAtMs as u, resolveJobLastRunStatus as v, resolveJobPayloadTextForMain as y };
