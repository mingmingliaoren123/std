...
import { D as resolveIntegerOption, p as finiteSecondsToTimerSafeMilliseconds } from "./number-coercion-CJQ8TR--.js";
...
import { l as resolveOpenClawStateSqlitePath } from "./openclaw-state-db-DzSsA9Ji.js";
import "./agent-scope-B2Pk_xhT.js";
import { a as isSubagentSessionKey, c as parseAgentSessionKey, r as isCronRunSessionKey } from "./session-key-utils-A-JGvyXu.js";
import { b as toAgentStoreSessionKey, f as normalizeOptionalAgentId, m as resolveEventSessionKey, p as resolveAgentIdFromSessionKey, u as normalizeAgentId } from "./session-key-VWT_xzM9.js";
import { a as resolveAgentDir, c as resolveDefaultAgentId, o as resolveAgentWorkspaceDir, r as resolveAgentConfig } from "./agent-scope-config-BxAUeF6t.js";
import { r as runCommandWithTimeout } from "./exec-DaeiOqVs.js";
import { H as resolveCronTriggerMinIntervalMs, i as getRuntimeConfig } from "./io-By0s-a_s.js";
...
import { _ as listSessionEntries, i as applySessionEntryLifecycleMutation, y as loadSessionEntry } from "./session-accessor-D7yi6P1i.js";
import "./logging-DBHEcFUB.js";
import { d as ensureAgentWorkspace } from "./workspace-DkQ7irPD.js";
import { u as resolveFailoverReasonFromError } from "./failover-error-B2zFYEnG.js";
import { a as isRetryableHeartbeatBusySkipReason, o as requestHeartbeat } from "./heartbeat-wake-DibZKDF_.js";
import { o as enqueueSystemEventEntry, t as consumeSelectedSystemEventEntries } from "./system-events-BfmWSF2P.js";
import { $ as trackActiveCronTaskRunSettlement, Q as startActiveCronTaskRunSettlementGrace, X as registerActiveCronTaskRun } from "./task-registry-Cws4vLl0.js";
import { a as failTaskRunByRunId, i as createRunningTaskRun, n as completeTaskRunByRunId } from "./detached-task-runtime-B93mVzDV.js";
import { f as saveCronQuarantineFile, g as normalizeCronJobIdentityFields, h as getInvalidPersistedCronJobReason, m as cronSchedulingInputsEqual, r as loadCronJobsStoreWithConfigJobs, s as resolveCronJobsStorePath, u as saveCronJobsStore } from "./store-ScQ9SjOe.js";
import { n as normalizeCronJobInput } from "./normalize-BMkddmz2.js";
import { a as resolveCronNotificationSessionKey, i as resolveCronDeliverySessionKey, n as isInvalidCronSessionTargetIdError, o as resolveCronSessionTargetSessionKey } from "./session-target-YpyFax1g.js";
import { t as abortAndDrainEmbeddedAgentRun } from "./runs-B0SQhu92.js";
...
import { i as enqueueCommandInLane } from "./command-queue-DqzpsN4m.js";
import { a as normalizeCronRunDiagnostics, n as createCronRunDiagnosticsFromError, o as summarizeCronRunDiagnostics } from "./run-diagnostics-B6oW4-9O.js";
import { r as normalizeCronRunLogJobId, s as resolveCronRunLogPruneOptions, t as appendCronRunLog } from "./run-log-DIhrTrSU.js";
...
import { r as resolveMainScopedEventSessionKey } from "./event-session-routing-Tqx9dDCC.js";
import { t as createOpenClawCodingTools } from "./agent-tools-BD8WL7ny.js";
import { m as createToolSearchCatalogRef, v as registerHeadlessToolSearchCatalog } from "./tool-search-B8B4Spes.js";
import { t as resolveToolLoopDetectionConfig } from "./tool-loop-detection-config-CmS_1rXy.js";
import { n as resolveCronStoredDeliveryContext } from "./delivery-context-Bo9jS5mk.js";
...
import { t as sendDurableMessageBatch } from "./send-CwUe7TNf.js";
import "./runtime-CkzezCbc.js";
import { a as preExecutionTimeoutErrorMessage, c as timeoutErrorMessage, i as normalizeCronRunErrorText, r as isSetupTimeoutErrorText, s as setupTimeoutErrorMessage, t as abortErrorMessage } from "./execution-errors-BeXudclS.js";
import { o as resolveSandboxContext } from "./sandbox-DtTssSMH.js";
import { a as runCodeModeScriptHeadless } from "./code-mode-D5mNEiYV.js";
import { t as getProcessSupervisor } from "./supervisor-AWzFqBCH.js";
import { r as resolveEmbeddedAttemptToolConstructionPlan, t as applyEmbeddedAttemptToolsAllow } from "./attempt-tool-construction-plan-B_ioY5W5.js";
...
import { t as createOutboundSendDeps } from "./outbound-send-deps-BkAAWpG5.js";
import { n as resolveAgentOutboundIdentity } from "./identity-D2mUYNGI.js";
import { n as resolveCronDeliveryPlan, r as resolveFailureDestination } from "./delivery-plan-BnWAdV0p.js";
import { _ as resolveJobErrorBackoffUntilMs, a as computeJobNextRunAtMs, b as resolveDeliveryTarget, c as errorBackoffMs, d as isJobDue, f as isJobEnabled, g as recordScheduleComputeError, h as recomputeNextRunsForMaintenance, i as assertSupportedJobSpec, l as findJobOrThrow, m as recomputeNextRuns, n as applyDeclarativeJobSpec, o as computeJobPreviousRunAtMs, p as nextWakeAtMs, r as applyJobPatch, s as createJob, t as DEFAULT_ERROR_BACKOFF_SCHEDULE_MS, u as hasScheduledNextRunAtMs, v as resolveJobLastRunStatus$1, y as resolveJobPayloadTextForMain } from "./jobs-qB_gTO89.js";
import { t as resolveCronAgentSessionKey } from "./session-key-BpcpZT5I.js";
import { n as buildCronAgentDefaultsConfig, r as resolveCronActiveRuntimeConfig, t as runCronIsolatedAgentTurn } from "./isolated-agent-B-VTFOj4.js";
import { t as createCronExecutionId } from "./run-id-kGde0n7U.js";
import { t as computeNextRunAtMs } from "./schedule-DFQuHnaZ.js";
import { i as tryFinishCronTaskRun, n as resolveMainSessionCronRunSessionKey, r as tryCreateCronTaskRun } from "./task-runs-BLjxY-br.js";
import { n as runHeartbeatOnce } from "./heartbeat-runner-DA5uXrsS.js";
...
function applyJobResult(state, job, result, opts) {
	...
	if (!shouldDelete) if (job.schedule.kind === "at") {
		...
		if (job.schedule.kind === "cron") {
			const minNext = result.endedAt + Math.max(MIN_REFIRE_GAP_MS, job.trigger ? resolveCronTriggerMinIntervalMs(state.deps.cronConfig) : 0);
		...
		} else job.state.nextRunAtMs = naturalNext !== void 0 && job.trigger ? Math.max(naturalNext, result.endedAt + resolveCronTriggerMinIntervalMs(state.deps.cronConfig)) : naturalNext;
...
function applyTriggerNoFireResult(state, job, result) {
	job.state.runningAtMs = void 0;
	job.updatedAtMs = result.endedAt;
	...
	try {
		...
		const floorMs = Math.max(MIN_REFIRE_GAP_MS, resolveCronTriggerMinIntervalMs(state.deps.cronConfig));
...
function applyOutcomeToStoredJob(state, result) {
	...
	if (!job) {
		if (result.status === "ok" && result.triggerEval?.fired === false) return;
		if (result.status === "ok") {
...
async function persistOrRestore(state, snapshot, postPersistAutoDisableNotifications = []) {
	try {
		await persist(state);
	} catch (err) {
	...
	for (const notify of postPersistAutoDisableNotifications) notify();
}
...
function finalizeUpdatedJob(params) {
	...
	if (schedulingInputsChanged) if (isJobEnabled(nextJob)) nextJob.state.nextRunAtMs = computeJobNextRunAtMs(nextJob, now);
	else {
...
async function add(state, input, opts) {
	return await locked(state, async () => {
		...
		if (normalizedId) normalizeCronRunLogJobId(normalizedId);
...
async function updateLoadedJob(params) {
	...
	finalizeUpdatedJob({
		...
		nextJob,
		now,
		schedulingInputsRequested: patch.schedule !== void 0 || patch.enabled !== void 0 || patch.trigger !== void 0,
...
async function prepareManualRun(state, id, mode, opts) {
	...
	return await locked(state, async () => {
		...
		if (opts?.payload) executionJob.payload = structuredClone(opts.payload);
		return {
			ok: true,
			ran: true,
			jobId: job.id,
...
function createCronServiceState(deps) {
	return {
		...
		pendingCatchupDeferralJobIds: /* @__PURE__ */ new Set(),
...
var TriggerEvaluationTimeoutError = class extends Error {
	constructor(message = "cron trigger evaluation timed out") {
		super(message);
		this.name = "TriggerEvaluationTimeoutError";
	}
};
function createTriggerDeadlineScope(externalSignal) {
	const controller = new AbortController();
	const onExternalAbort = () => controller.abort(new TriggerEvaluationTimeoutError("cron trigger evaluation aborted"));
	externalSignal?.addEventListener("abort", onExternalAbort, { once: true });
	if (externalSignal?.aborted) onExternalAbort();
	const timer = setTimeout(() => controller.abort(new TriggerEvaluationTimeoutError()), HEADLESS_TRIGGER_WALL_CLOCK_MS);
	return {
...
async function awaitTriggerSignal(promise, signal) {
	if (signal.aborted) throw signal.reason instanceof Error ? signal.reason : new TriggerEvaluationTimeoutError();
	let onAbort;
	try {
		const aborted = new Promise((_resolve, reject) => {
			onAbort = () => reject(signal.reason instanceof Error ? signal.reason : new TriggerEvaluationTimeoutError());
...
function createCronTriggerEvaluator(deps) {
	...
	return async function evaluateCronTrigger(params) {
		...
		try {
			...
			if (remainingWallClockMs <= 0) throw new TriggerEvaluationTimeoutError();
...
function buildGatewayCronService(params) {
	...
	const triggerEvaluator = params.cfg.cron?.triggers?.enabled === true ? createCronTriggerEvaluator({ config: params.cfg }) : void 0;
	...
	const cron = new CronService({
		...
		...triggerEvaluator ? { evaluateCronTrigger: ({ job, script, state, abortSignal }) => triggerEvaluator({
			jobId: job.id,
			agentId: job.agentId,
			script,
			state,
			toolsAllow: job.payload.kind === "agentTurn" ? job.payload.toolsAllow : void 0,
			abortSignal
		}) } : {},
		defaultAgentId,
		...
		onEvent: (evt) => {
			...
			if (evt.action === "added" || evt.action === "updated" || evt.action === "removed") reconcileExitWatchers();
	...
	exitWatchersRef.current = createCronExitWatchers({
		...
		fireOnExit: (job, exit) => fireOnExitJob(job, exit, { run: (jobId, payload) => cron.run(jobId, "force", payload ? { payload } : void 0) }),
		logger: cronLogger
	});
