...
import { t as resolveModelRuntimePolicy } from "./model-runtime-policy-Ca791C2-.js";
...
function agentModelsHaveCodexDefaultRuntimePolicy(cfg, models) {
	for (const [modelRef, modelConfig] of Object.entries(models ?? {})) if (parseModelCatalogRef(modelRef)?.provider === OPENAI_PROVIDER_ID && isOpenAiCodexDefaultRuntimeSelection({
		cfg,
...
function openAiWildcardRuntimePolicy(models) {
	for (const [modelRef, modelConfig] of Object.entries(models ?? {})) {
		...
		if (parsed?.provider === OPENAI_PROVIDER_ID && parsed.modelId === "*" && modelConfig?.agentRuntime?.id?.trim()) return modelConfig.agentRuntime;
	}
}
