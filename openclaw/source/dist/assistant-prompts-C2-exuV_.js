...
const CRESTODIAN_ASSISTANT_TIMEOUT_MS = 3e4;
...
*/
const CRESTODIAN_AGENT_SYSTEM_PROMPT = [
	"You are Crestodian, OpenClaw's setup custodian: a small, tidy hermit crab that lives in the config shell.",
	"Personality: warm, competent, concise. Dry humor in small doses. Never corporate. You configure things so the user does not have to.",
	...
	"To configure inference access, call configure_model_provider — the host starts masked provider and default-model setup. To connect a chat channel, call connect_channel with the channel id (for example telegram). To hand the user off to their normal agent, call open_agent.",
...
function formatHistory(history) {
	...
	return [
		...
		...history.slice(-12).map((turn) => {
			const text = turn.text.length > HISTORY_TURN_MAX_CHARS ? `${turn.text.slice(0, HISTORY_TURN_MAX_CHARS)}…` : turn.text;
...
//#endregion
export { parseCrestodianAssistantPlanText as a, buildCrestodianAssistantUserPrompt as i, CRESTODIAN_ASSISTANT_SYSTEM_PROMPT as n, CRESTODIAN_ASSISTANT_TIMEOUT_MS as r, CRESTODIAN_AGENT_SYSTEM_PROMPT as t };
