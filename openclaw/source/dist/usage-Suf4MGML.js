...
import { ct as resolveSessionStoreAgentId, ut as resolveStoredSessionKeyForAgentStore } from "./store-BJJhlPrk.js";
...
import { o as listAgentsForGateway, u as loadSessionEntry } from "./session-utils-DD3pe_2A.js";
import { t as resolvePreferredSessionKeyForSessionIdMatches } from "./session-id-resolution-Y7V2bVg1.js";
import { mn as validateSessionsUsageParams, t as formatValidationErrors } from "./src-CToKmqGn.js";
import { Gn as errorShape, Wn as ErrorCodes } from "./schema-BuOFpc7K.js";
import "./provider-usage-DfdzHObI.js";
import { t as loadProviderUsageSummary } from "./provider-usage.load-BNEsPrNj.js";
import { c as resolveExistingUsageSessionFile, i as loadSessionCostSummariesFromCache, l as addCostUsageTotals, o as loadSessionLogs, r as loadCostUsageSummaryFromCache, s as loadSessionUsageTimeSeries, t as discoverAllSessions, u as createEmptyCostUsageTotals } from "./session-cost-usage-B0dBxiXW.js";
...
const datePartsToStartMs = (parts, interpretation) => {
	...
	if (interpretation.mode === "specific") return Date.UTC(year, monthIndex, day) - interpretation.utcOffsetMinutes * 60 * 1e3;
	return Date.UTC(year, monthIndex, day);
};
const datePartsToEndMs = (parts, interpretation) => datePartsToStartMs(shiftDateParts(parts, 1), interpretation) - 1;
...
const resolveDayBucketUtcOffsetMinutes = (interpretation) => interpretation.mode === "gateway" ? void 0 : interpretation.mode === "specific" ? interpretation.utcOffsetMinutes : 0;
...
const resolveDateRange = (params) => {
	...
	if (startDateParts && endDateParts) {
		...
		if (startMs > datePartsToStartMs(endDateParts, interpretation)) return {
			ok: false,
	...
	if (rangeDays !== void 0) return {
		ok: true,
		value: resolveTrailingDays(todayDateParts, rangeDays, interpretation)
	};
	const days = parseDays(params.days);
	if (days !== void 0) return {
		ok: true,
		value: resolveTrailingDays(todayDateParts, Math.max(1, days), interpretation)
	};
	return {
		ok: true,
		value: resolveTrailingDays(todayDateParts, 30, interpretation)
	};
};
...
async function loadAllAgentCostUsageSummary(params) {
	const agentLoadResult = await runTasksWithConcurrency({
		tasks: listAgentsForGateway(params.config).agents.map((agent) => normalizeAgentId(agent.id)).map((agentId) => () => loadCostUsageSummaryFromCache({
			startMs: params.startMs,
			endMs: params.endMs,
			dailyUtcOffsetMinutes: params.dailyUtcOffsetMinutes,
			config: params.config,
	...
	if (agentLoadResult.hasError) throw agentLoadResult.firstError;
