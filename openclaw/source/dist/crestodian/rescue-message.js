...
import { a as parseCrestodianOperation, i as isPersistentCrestodianOperation, n as executeCrestodianOperation, r as formatCrestodianPersistentPlan } from "../operations-BqXhCFby.js";
...
function formatUnsupportedRemoteOperation(operation) {
	...
	if (operation.kind === "channel-setup") return ["Crestodian rescue cannot host the interactive channel setup from a message channel.", "Run `openclaw crestodian` locally and say `connect " + operation.channel + "` instead."].join(" ");
	if (operation.kind === "model-setup") return ["Crestodian rescue cannot host model-provider credential setup from a message channel.", "Run `openclaw crestodian` locally and say `configure model provider` instead."].join(" ");
...
async function runCrestodianRescueMessage(input) {
	...
	if (classifyCrestodianApprovalText(rescueMessage) === "decline") {
		const pending = await readPending(pendingPath);
		await fs.rm(pendingPath, { force: true });
		return pending ? "Dropped the pending Crestodian rescue change." : "No pending Crestodian rescue change is waiting for approval.";
	}
