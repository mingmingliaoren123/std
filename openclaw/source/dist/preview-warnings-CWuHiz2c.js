...
import { n as isToolAllowedByPolicyName, t as isToolAllowedByPolicies } from "./tool-policy-match-CgU98OQh.js";
...
import { t as resolveDoctorPrimaryModelRef } from "./primary-model-ref-DcuBG2yS.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-IFftPDvh.js"));
...
async function resolveDoctorChannelPreviewConfig(params) {
	const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] = await Promise.all([import("./command-secret-gateway-Ch_Vp-Mi.js"), import("./command-secret-targets-Bbo5uDG2.js")]);
...
async function collectDoctorPreviewNotes(params) {
	...
	const { collectBlockedLegacyOpenAICodexProviderWarnings } = await import("./legacy-config-migrations.runtime.models-BRIax6C5.js");
	warnings.push(...collectBlockedLegacyOpenAICodexProviderWarnings(params.cfg));
	const { collectActiveToolSchemaProjectionWarnings } = await import("./active-tool-schema-warnings-5Zjf_B6S.js");
	warnings.push(...collectActiveToolSchemaProjectionWarnings({
		cfg: params.cfg,
		env
	}));
	const channelPluginRuntime = await import("./channel-plugin-blockers-Boue9EUL.js");
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-CEMwLZ1-.js");
	...
	if (hasPluginConfig) {
		const { collectCodexRouteWarnings } = await import("./codex-route-warnings-BDOV1lrY.js");
		warnings.push(...collectCodexRouteWarnings({
			cfg: params.cfg,
			env
		}));
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-F5cRO8o4.js");
	...
	if (hasSubagentAllowlistConfig(params.cfg)) {
		const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } = await import("./stale-subagent-allowlist-DRLUAdQz.js");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-DsbkmkIV.js");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-BCeGgj50.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-CTpqYPYK.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-CijjGqUL.js");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-B1y0y3k-.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-DuBfbXsu.js");
...
//#endregion
export { resolveDoctorChannelPreviewConfig as a, collectVisibleReplyToolPolicyWarnings as i, collectDoctorPreviewNotes as n, collectProfileConfiguredToolSectionWarnings as r, collectChannelBoundMessageToolPolicyWarnings as t };
