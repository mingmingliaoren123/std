...
import { c as redactSensitiveText, u as redactToolPayloadText } from "./redact-B9QQ4Wyz.js";
...
import { _ as resolvePinnedHostnameWithPolicy, d as isPrivateNetworkAllowedByPolicy, p as matchesHostnameAllowlist } from "./ssrf-BayeDjCv.js";
import { r as ensurePortAvailable } from "./ports-3JAu4m81.js";
...
import { r as saveJsonFile, t as loadJsonFile } from "./json-store-BGu-jw3d.js";
import "./process-runtime-KP-LXN4B.js";
import { n as redactCdpUrl } from "./browser-config-DCrASvM0.js";
import "./provider-http-CwvZqS_e.js";
import { A as CHROME_LAUNCH_READY_WINDOW_MS, M as CHROME_STDERR_HINT_MAX_CHARS, N as CHROME_STOP_TIMEOUT_MS, O as CHROME_BOOTSTRAP_EXIT_TIMEOUT_MS, Q as ensureAbsoluteDirectory, X as assertManagedProxyAllowsCdpUrl, Y as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, b as BrowserProfileUnavailableError, c as isWebSocketUrl, d as scopeCdpPolicyToConfiguredEndpoint, f as withCdpSocket, g as BrowserCdpEndpointBlockedError, i as fetchJson, k as CHROME_BOOTSTRAP_PREFS_TIMEOUT_MS, l as normalizeCdpHttpBaseForJsonEndpoints, n as assertCdpEndpointAllowed, r as fetchCdpChecked, s as isDirectCdpWebSocketEndpoint, t as appendCdpPath, u as openCdpWebSocket } from "./cdp.helpers-B9jipAGQ.js";
import { a as DEFAULT_DOWNLOAD_DIR, r as resolveManagedBrowserHeadlessMode, t as getManagedBrowserMissingDisplayError } from "./config-DpWXcVmn.js";
import "./tmp-openclaw-dir-DRHs_iAK.js";
import { r as resolveBrowserExecutableForPlatform } from "./chrome.executables-DP_XzlNl.js";
import "./redact-AImnGvyf.js";
...
async function requestGracefulChromeClose(cdpPort, timeoutMs) {
	const commandTimeoutMs = Math.max(1, Math.min(timeoutMs, CHROME_GRACEFUL_CLOSE_COMMAND_TIMEOUT_MS));
	let commandSent = false;
	try {
		const wsUrl = await getChromeWebSocketUrl(cdpUrlForPort(cdpPort), Math.min(commandTimeoutMs, 200));
...
async function waitForChromeCdpShutdown(cdpPort, timeoutMs) {
	const start = Date.now();
	while (Date.now() - start < timeoutMs) {
		if (!await isChromeReachable(cdpUrlForPort(cdpPort), 200)) return true;
		const remainingMs = timeoutMs - (Date.now() - start);
		await new Promise((resolve) => {
			setTimeout(resolve, Math.max(1, Math.min(100, remainingMs)));
		});
	}
	return !await isChromeReachable(cdpUrlForPort(cdpPort), 200);
}
...
async function stopOpenClawChrome(running, timeoutMs = CHROME_STOP_TIMEOUT_MS) {
	...
	try {
		...
		if (await requestGracefulChromeClose(running.cdpPort, timeoutMs) && await waitForChromeCdpShutdown(running.cdpPort, timeoutMs)) return;
		try {
			proc.kill("SIGTERM");
		} catch {}
		if (await waitForChromeCdpShutdown(running.cdpPort, timeoutMs)) return;
		try {
			proc.kill("SIGKILL");
		} catch {}
	} finally {
