...
import { r as stripInboundMetadata, t as extractInboundSenderLabel } from "./strip-inbound-meta-DMvJPx9F.js";
import { n as extractAssistantVisibleText } from "./chat-message-content-CeBHi_A4.js";
import { i as hasNonzeroUsage, o as normalizeUsage, r as deriveSessionTotalTokens } from "./usage-BJjt0RVM.js";
import { r as jsonUtf8Bytes } from "./json-utf8-bytes-C14lActR.js";
import { i as hasInterSessionUserProvenance } from "./input-provenance-BkJA7lbx.js";
...
import { a as resolveSessionTranscriptResetArchiveCandidatesAsync, i as resolveSessionTranscriptCandidates } from "./session-transcript-files.fs-Cj46Z9Oy.js";
...
async function readSessionMessagesPageWithStatsAsync(sessionId, storePath, sessionFile, opts, agentId) {
	const filePath = opts.allowResetArchiveFallback === true ? await findExistingTranscriptHistoryPathAsync(sessionId, storePath, sessionFile, agentId) : findExistingTranscriptPath(sessionId, storePath, sessionFile, agentId);
	if (!filePath) return {
		messages: [],
		totalMessages: 0
...
function hasTranscriptUsage(snapshot) {
	return Boolean(snapshot && (snapshot.contextUsage !== void 0 || snapshot.inputTokens !== void 0 || snapshot.outputTokens !== void 0 || snapshot.cacheRead !== void 0 || snapshot.cacheWrite !== void 0 || snapshot.totalTokens !== void 0 || snapshot.costUsd !== void 0));
}
function extractLatestUsageFromTranscriptLines(lines) {
	...
	const selected = selectBoundedActiveTailRecords(parsed.map(({ entry }) => entry), { failClosedOnInvalidLeafControl: true });
	...
	for (const entry of selected) {
		...
		if (hasTranscriptUsage(current)) {
			latest = current;
			trailingBytes = 0;
		} else if (latest) trailingBytes += Buffer.byteLength(line, "utf8") + 1;
	}
	if (latest) latest.trailingBytes = trailingBytes;
	return latest;
}
function hasInvalidLeafControl(lines) {
	return scanSessionTranscriptTree(Array.from(lines).flatMap((line) => {
		const entry = parseTailTranscriptRecord(line);
		return entry ? [entry.record] : [];
	})).hasInvalidLeafControl;
}
async function extractLatestUsageFromTranscriptIndex(filePath) {
	const index = await readSessionTranscriptIndex(filePath);
...
async function readLatestRecentSessionUsageFromTranscriptAsync(sessionId, storePath, sessionFile, agentId, maxBytes) {
	...
	try {
		...
		if (hasInvalidLeafControl(lines)) return await extractLatestUsageFromTranscriptIndex(filePath);
...
//#endregion
export { stripEnvelope as C, stripInternalMetadataForDisplay as S, visitSessionMessagesAsync as _, readRecentSessionMessagesWithStatsAsync as a, countToolResults as b, readSessionMessageCountAsync as c, readSessionMessagesPageWithStatsAsync as d, readSessionMessagesWithSourceAsync as f, resolveSessionHistoryTranscriptPathAsync as g, readSessionTitleFieldsFromTranscriptAsync as h, readRecentSessionMessagesAsync as i, readSessionMessages as l, readSessionTitleFieldsFromTranscript as m, capArrayByJsonBytes as n, readRecentSessionUsageFromTranscript as o, readSessionPreviewItemsFromTranscript as p, readLatestRecentSessionUsageFromTranscriptAsync as r, readSessionMessageByIdAsync as s, attachOpenClawTranscriptMeta as t, readSessionMessagesAsync as u, stripEnvelopeFromMessage as v, stripMessageIdHints as w, extractToolCallNames as x, stripEnvelopeFromMessages as y };
