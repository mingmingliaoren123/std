...
function describeSessionsListTool() {
	return ["List visible sessions; filter by kind, label, agentId, search, activity, archive state.", "Use before sessions_history or sessions_send target selection."].join(" ");
}
/** Describes the sessions_history tool for model-facing instructions. */
function describeSessionsHistoryTool() {
	return ["Fetch sanitized history for visible session.", "Use before replying, debugging, resuming; supports limit, offset pagination, and tool-message inclusion."].join(" ");
}
