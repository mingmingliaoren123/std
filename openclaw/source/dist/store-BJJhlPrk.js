...
import { a as resolveMainSessionKey, n as canonicalizeMainSessionAlias } from "./main-session-D7Jmp9DO.js";
import { a as normalizeDeliveryChannelRoute, i as mergeDeliveryContext, n as deliveryContextFromSession, o as normalizeDeliveryContext, s as normalizeSessionDeliveryFields, t as deliveryContextFromChannelRoute } from "./delivery-context.shared-3o3tBaCD.js";
import { n as isInternalNonDeliveryChannel } from "./message-channel-constants-BXOA4cxJ.js";
import { _ as isSessionArchiveArtifactName, a as resolveSessionFilePath, d as resolveStorePath, f as validateSessionId, g as isPrimarySessionTranscriptFileName, h as isCompactionCheckpointTranscriptFileName, i as resolveExplicitSessionFilePath, m as formatSessionArchiveTimestamp, p as SESSION_STORE_TEMP_STALE_MS, v as isSessionStoreTempArtifactName, x as isTrajectorySessionArtifactName } from "./paths-C2C4lJH6.js";
...
import { a as resolveTrajectoryPointerFilePath, i as resolveTrajectoryFilePath } from "./paths-B4qcz6gV.js";
import { a as projectSessionStoreForPersistence, i as isSessionSkillPromptBlobReadable, n as ensureSessionStorePromptBlobsForPersistence, r as hydrateSessionStoreSkillPromptRefs, t as clearSessionSkillPromptRefCache } from "./skill-prompt-blobs-zJRX9N65.js";
import { a as normalizeChannelId, i as listChannelPlugins, n as getLoadedChannelPlugin } from "./registry-CCU9J1Ai.js";
import { i as resolveSessionThreadInfo } from "./session-conversation-Sco2Iyil.js";
...
import { i as normalizeMessageChannel, r as listDeliverableMessageChannels } from "./message-channel-normalize-s5PUEeqJ.js";
...
import { n as runQueuedStoreWrite, t as clearStoreWriterQueuesForTest } from "./store-writer-queue-DPKo3FoW.js";
...
const loadTrajectoryCleanupRuntime = createLazyRuntimeModule(() => import("./cleanup-D3qukyYN.js"));
...
function listSessionEntries(options = {}) {
	return readSessionEntries(resolveSessionStorePathForScope(options)).map(([sessionKey, entry]) => ({
		sessionKey,
...
function resolveLifecyclePrimaryEntry(params) {
	...
	if (primaryEntry) params.store[params.target.canonicalKey] = primaryEntry;
...
function resolveLifecyclePrimaryEntrySnapshot(params) {
	...
	if (freshestMatch && (!currentPrimary || (freshestMatch.entry.updatedAt ?? 0) > (currentPrimary.updatedAt ?? 0))) return freshestMatch.entry;
...
async function resetSessionEntryLifecycle(params) {
	return await runExclusiveSessionStoreWrite(params.storePath, async () => {
		...
		const reusesTranscriptPath = previousSessionFile !== void 0 && normalizePathForLifecycleComparison(previousSessionFile) === normalizePathForLifecycleComparison(nextSessionFile);
		if (!reusesTranscriptPath) ensureLifecycleTranscriptHeader({
...
async function deleteSessionEntryLifecycle(params) {
	return await runExclusiveSessionStoreWrite(params.storePath, async () => {
		...
		const expectedEntryMatches = params.expectedEntry === void 0 || JSON.stringify(deletedEntry) === JSON.stringify(params.expectedEntry);
		const expectedLifecycleRevisionMatches = params.expectedLifecycleRevision === void 0 || deletedEntry.lifecycleRevision === params.expectedLifecycleRevision;
		const expectedSessionIdMatches = !params.expectedSessionId || deletedEntry.sessionId === params.expectedSessionId || deletedEntry.sessionId === void 0 && params.expectedLifecycleRevision !== void 0 && expectedLifecycleRevisionMatches;
		const expectedUpdatedAtMatches = params.expectedUpdatedAt === void 0 || deletedEntry.updatedAt === params.expectedUpdatedAt;
		if (!expectedEntryMatches || !expectedLifecycleRevisionMatches || !expectedSessionIdMatches || !expectedUpdatedAtMatches) {
			restoreUnchangedSessionStoreCache(params.storePath, store);
			return {
				archivedTranscripts: [],
				deleted: false,
...
async function updateLastRoute(params) {
	...
	return await runExclusiveSessionStoreWrite(storePath, async () => {
		...
		const next = mergeSessionEntryPreserveActivity(existing, metaPatch ? {
			...basePatch,
			...metaPatch
		} : basePatch);
		return await persistResolvedSessionEntry({
...
//#endregion
export { parseSessionThreadInfo as $, foldedSessionKeyAliasCandidates as A, buildGroupDisplayName as B, readSessionEntries as C, collectSessionMaintenancePreserveKeys as D, resolveMaintenanceConfig as E, cloneSessionStoreRecord as F, capEntryCount as G, enforceSessionDiskBudget as H, getSessionStoreCacheVersion as I, pruneStaleEntries as J, getActiveSessionMaintenanceWarning as K, resolveSessionStorePathForScope as L, isConfirmedLowercasedLegacyAlias as M, normalizeStoreSessionKey as N, registerSessionMaintenancePreserveKeysProvider as O, resolveSessionStoreEntry as P, shouldRunModelRunPrune as Q, deriveSessionMetaPatch as R, loadSessionStore as S, readSessionStoreSnapshot as T, pruneUnreferencedSessionArtifacts as U, resolveGroupSessionKey as V, resolveSessionArtifactCanonicalPathsForEntry as W, resolveMaintenanceConfigFromInput as X, pruneStaleModelRunEntries as Y, resolveQuotaSuspensionEntryMaintenance as Z, updateLastRoute as _, cleanupSessionLifecycleArtifacts as a, normalizeSessionEntrySlotKey as at, runExclusiveSessionStoreWrite as b, listSessionEntries as c, resolveSessionStoreAgentId as ct, projectSessionEntryForPersistenceRevision as d, resolveStoredSessionOwnerAgentId as dt, parseSessionThreadInfoFast as et, purgeDeletedAgentSessionEntries as f, saveSessionStore as g, resetSessionEntryLifecycle as h, archiveRemovedSessionTranscripts as i, resolveCacheTtlMs as it, hasMismatchedCaseSensitiveDeliveryProof as j, normalizePersistedSessionEntryShape as k, patchSessionEntry as l, resolveSessionStoreKey as lt, recordSessionMetaFromInbound as m, applySessionEntryPatchProjection as n, createExpiringMapCache as nt, deleteSessionEntryLifecycle as o, canonicalizeSessionKeyForAgent as ot, readSessionUpdatedAt as p, normalizeResolvedMaintenanceConfigInput as q, applySessionStoreEntryPatch as r, isCacheEnabled as rt, getSessionEntry as s, canonicalizeSpawnedByForAgent as st, applySessionEntryLifecycleMutation as t, resolveLoadedSessionThreadInfo as tt, patchSessionEntryWithKey as u, resolveStoredSessionKeyForAgentStore as ut, updateSessionStore as v, readSessionEntry as w, clearSessionStoreCacheForTest as x, updateSessionStoreEntry as y, snapshotSessionOrigin as z };
