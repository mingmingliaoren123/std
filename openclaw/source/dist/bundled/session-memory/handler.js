...
import { b as toAgentStoreSessionKey, p as resolveAgentIdFromSessionKey } from "../../session-key-VWT_xzM9.js";
import { o as resolveAgentWorkspaceDir } from "../../agent-scope-config-BxAUeF6t.js";
import { nt as isOpenClawDeliveryMirrorAssistantMessage } from "../../session-accessor-D7yi6P1i.js";
...
import { t as generateSlugViaLLM } from "../../llm-slug-generator-CS9BOc_o.js";
import { r as resolveHookConfig } from "../../config-D9HgDUPt.js";
		...
		for (const line of lines) try {
			...
			if (entry.type === "message" && entry.message) {
				...
				if ((role === "user" || role === "assistant") && "content" in msg && msg.content) {
					...
					if (role === "user") lastAssistantText = void 0;
					...
					if (isOpenClawDeliveryMirrorAssistantMessage(msg)) {
						if (sanitized && sanitized === lastAssistantText) continue;
					}
					if (sanitized && !sanitized.startsWith("/")) {
						allMessages.push(`${role}: ${sanitized}`);
						if (role === "assistant") lastAssistantText = sanitized;
					}
				}
			}
		} catch {}
...
async function findPreviousSessionFile(params) {
	try {
		...
		if (trimmedSessionId) {
			...
			const canonicalResetVariants = files.filter((name) => name.startsWith(`${canonicalFile}.reset.`)).toSorted().toReversed();
			if (canonicalResetVariants.length > 0) return path.join(params.sessionsDir, canonicalResetVariants[0]);
			...
			const topicResetVariants = files.filter((name) => name.startsWith(`${trimmedSessionId}-topic-`) && name.includes(".jsonl.reset.")).toSorted().toReversed();
			if (topicResetVariants.length > 0) return path.join(params.sessionsDir, topicResetVariants[0]);
		}
		...
		const resetJsonl = files.filter((name) => name.includes(".jsonl.reset.")).toSorted().toReversed();
		if (resetJsonl.length > 0) return path.join(params.sessionsDir, resetJsonl[0]);
	} catch {}
}
//#endregion
