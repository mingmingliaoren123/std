...
import { c as callGateway, g as isGatewayTransportError } from "./call-Bj6Erfmh.js";
import "./message-channel-CB9y2CYk.js";
import { i as serializeSessionCleanupResult, n as resolveSessionCleanupAction, r as runSessionsCleanup } from "./sessions-oDygYVdy.js";
import { t as resolveSessionStoreTargetsOrExit } from "./session-store-targets-BdQM-mnu.js";
import { c as resolveSessionDisplayModel, i as formatSessionModelCell, n as formatSessionFlagsCell, o as toSessionDisplayRows, r as formatSessionKeyCell, t as formatSessionAgeCell } from "./sessions-table-CUsZDlFY.js";
...
function formatCleanupActionCell(action, rich) {
	...
	if (action === "repair-session-file") return theme.accentBright(label);
...
function buildActionRows(params) {
	return toSessionDisplayRows(params.beforeStore).map((row) => Object.assign({}, row, {
		...
		action: resolveSessionCleanupAction({
			key: row.key,
			repairedKeys: params.repairedKeys ?? /* @__PURE__ */ new Set(),
...
function buildLabelSummaries(actionRows) {
	...
	for (const actionRow of actionRows) {
		...
		if (actionRow.action === "keep" || actionRow.action === "repair-session-file") summary.kept += 1;
