...
const MATRIX_STREAM_ERROR_KILL_GRACE_MS = 1e3;
...
async function runFixedCommandWithTimeout(params) {
	return await new Promise((resolve) => {
		...
		const failReadableStream = (streamName) => (error) => {
			if (settled || streamErrorMessage) return;
		...
		proc.on("close", (code) => {
			const streamErrorStderr = streamErrorMessage ? stderr ? appendBoundedOutputTail(stderr, `\n${streamErrorMessage}`) : streamErrorMessage : stderr;
			finalize({
