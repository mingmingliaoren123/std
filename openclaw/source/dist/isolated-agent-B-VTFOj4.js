...
import { x as resolveSubagentModelConfigSelectionResult } from "./agent-scope-B2Pk_xhT.js";
...
import { _ as releaseAgentRunContext, d as getAgentEventLifecycleGeneration, f as getAgentRunContext, r as claimAgentRunContext, t as assertAgentRunLifecycleGenerationCurrent } from "./agent-events-CRggPZCM.js";
import { s as resolveSessionTranscriptPath } from "./paths-C2C4lJH6.js";
import { d as setSessionRuntimeModel } from "./types-CoDcFuoc.js";
import { S as findModelInCatalog, _ as resolveConfiguredModelRef, f as normalizeModelSelection, y as resolveHooksGmailModel } from "./model-selection-shared-iHJcI8fT.js";
import { n as resolveAllowedModelRef, t as getModelRefStatus } from "./model-selection-resolve-D2fjCpfo.js";
...
import { f as expandToolGroups, m as normalizeToolName } from "./tool-policy-BHUGxE3p.js";
...
import { i as resolveCronDeliverySessionKey } from "./session-target-YpyFax1g.js";
import { c as resolveSessionWorkStartError } from "./lifecycle-BS_t5emX.js";
import { i as hasNonzeroUsage, r as deriveSessionTotalTokens, t as deriveContextPromptTokens } from "./usage-BJjt0RVM.js";
...
import { d as isCommandLaneTaskTimeoutError } from "./command-queue-DqzpsN4m.js";
import { i as mergeCronRunDiagnostics, n as createCronRunDiagnosticsFromError, r as createCronRunDiagnosticsFromMissingWebSearchProvider, s as toolsAllowRequestsWebSearch, t as createCronRunDiagnosticsFromAgentResult } from "./run-diagnostics-B6oW4-9O.js";
import { l as retireSessionMcpRuntime } from "./agent-bundle-mcp-runtime--G82BMQs.js";
import "./agent-bundle-mcp-tools-CYL4t7pb.js";
import { g as resolveSourceDeliveryOutcome } from "./delivery-evidence-Du4oIHR6.js";
import { o as resolveCronAbortReasonText } from "./execution-errors-BeXudclS.js";
...
import { i as resolveCronPreflightCandidates, s as resolveCronChannelOutputPolicy, t as resolveCronSourceDeliveryPlan } from "./source-delivery-fallback-V9FLr_MZ.js";
import { a as persistCronSkillsSnapshotIfChanged, f as resolveCronPayloadOutcome, i as markCronSessionPreRun, l as isHeartbeatOnlyResponse, n as adoptCronRunSessionMetadata, o as projectCronOwnershipFields, p as resolveHeartbeatAckMaxChars, r as createPersistCronSessionEntry, s as resolveCronLifecycleRevisionIdentity, t as CronSessionLifecycleClaimError } from "./run-session-state-r5DnSgVq.js";
import { t as cleanupCronRunSessionAfterRun } from "./session-cleanup-DDx3rvwz.js";
import { n as resolveCronSession, t as loadCronSessionEntryLatest } from "./session-unZCiGt9.js";
...
const codexNativeWebSearchLoader = createLazyImportLoader(() => import("./codex-native-web-search-qgLpqH45.js"));
const webSearchRuntimeLoader = createLazyImportLoader(() => import("./runtime-f9uSIegK.js"));
...
async function createCronToolsAllowPreflightDiagnostics(params) {
	...
	try {
		...
		return createCronRunDiagnosticsFromMissingWebSearchProvider({
			toolsAllow,
			hasWebSearchProvider: Boolean(resolveWebSearchProviderId({
				config: params.cfg,
				agentDir: params.agentDir,
		...
		logWarn(`[cron:${params.jobId}] Failed to inspect web_search providers for toolsAllow diagnostics: ${String(error)}`);
		return;
	}
}
...
async function loadUsageFormatRuntime() {
	return await import("./usage-format-BAGC_u9L.js");
}
...
async function finalizeCronRun(params) {
	...
	if (hasNonzeroUsage(usage)) {
		...
		const totalTokens = typeof lastCallTotalTokens === "number" && lastCallTotalTokens > 0 ? lastCallTotalTokens : lastCallUsage?.contextUsage?.state === "unavailable" ? void 0 : deriveSessionTotalTokens({
			usage,
			contextTokens,
			promptTokens
		});
	...
	const resolveRunOutcome = (result) => prepared.withRunSession({
		...
		diagnostics: hasFatalErrorPayload ? mergeCronRunDiagnostics(runDiagnostics, createCronRunDiagnosticsFromError("agent-run", embeddedRunError ?? "cron isolated run returned an error payload")) : runDiagnostics,
		...telemetry
	});
	...
	if (deliveryResult.result) {
		const resultWithDeliveryMeta = {
			...
			delivery: deliveryTrace,
			diagnostics: mergeCronRunDiagnostics(runDiagnostics, deliveryResult.result.diagnostics, deliveryResult.result.status === "error" && deliveryResult.result.error ? createCronRunDiagnosticsFromError("delivery", deliveryResult.result.error) : void 0)
		};
