import { r as isVitestRuntimeEnv } from "./env-CKdem44B.js";
...
import { r as startHeartbeatRunner } from "./heartbeat-runner-DA5uXrsS.js";
import { r as createNoopHeartbeatRunner } from "./server-runtime-startup-services-BI_KXCQu.js";
...
function startGatewayCronWithLogging(params) {
	params.cron.start().then(() => params.afterStart?.()).catch((err) => params.logCron.error(`failed to start: ${String(err)}`));
}
...
function recoverPendingOutboundDeliveries(params) {
	(async () => {
		const { recoverPendingDeliveries } = await import("./delivery-queue-Biw7nk9t.js");
		const { deliverOutboundPayloadsInternal } = await import("./deliver-CwVHxrZJ.js");
...
function recoverPendingSessionDeliveries(params) {
	setTimeout(() => {
		(async () => {
			const { recoverPendingRestartContinuationDeliveries } = await import("./server-restart-sentinel-cj6qAdAU.js");
...
function startGatewayModelPricingRefreshOnDemand(params) {
	...
	let stopRefresh;
	(async () => {
		const { startGatewayModelPricingRefresh } = await import("./model-pricing-cache-DMOhLwXx.js");
		if (stopped) return;
