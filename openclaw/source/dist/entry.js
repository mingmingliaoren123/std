...
import { c as hasFlag, d as isRootHelpInvocation, f as isRootVersionInvocation, n as getCommandPathWithRootOptions, r as getCommandPositionalsWithRootOptions } from "./argv-APLDYHWW.js";
import { t as isMainModule } from "./is-main-CH4EEB_R.js";
import { t as resolveCliArgvInvocation } from "./argv-invocation-DJKlVa-q.js";
import { i as resolvePrecomputedSubcommandHelpCommand, n as applyCliProfileEnv, o as parseCliContainerArgs, r as parseCliProfileArgs, s as resolveCliContainerTarget, t as createGatewayStartupTrace } from "./startup-trace-Bc2ebu8Y.js";
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-BmsGkjsI.js"), import("./git-commit-tO78EJeA.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
if (!isMainModule({
	...
	if (!respawnWithoutOpenClawCompileCacheIfNeeded({
		...
		normalizeEnv();
		const { assertSupportedRuntime } = await import("./runtime-guard-BZ9MJdn_.js");
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-DIxn7ZuZ.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
		handleError(error);
		return true;
...
async function runMainOrRootHelp(argv) {
	...
	try {
		...
		const { formatCliFailureLines } = await import("./failure-output-CNKwM_Fv.js");
