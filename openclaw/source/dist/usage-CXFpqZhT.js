...
import { m as updateAuthProfileStoreWithLock, p as saveAuthProfileStore } from "./store-DH33UrUj.js";
...
function shouldProbeWhamForFailure(profile, reason) {
	...
	return profile?.type === "oauth" && Boolean(profile.access) && normalizedProvider === "openai" && (reason === "rate_limit" || reason === "empty_response" || reason === "no_error_details" || reason === "unclassified" || reason === "unknown");
}
...
async function markAuthProfileFailure(params) {
	...
	const currentWhamResult = whamResult && isSameWhamCredential(profile, store.profiles[profileId]) ? whamResult : null;
	if (reason === "no_error_details" && !currentWhamResult) return;
	const now = Date.now();
	const cfgResolved = resolveAuthCooldownConfig({
		cfg,
