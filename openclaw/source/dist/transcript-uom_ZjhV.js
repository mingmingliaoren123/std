...
import { a as resolveSessionFilePath, d as resolveStorePath, r as resolveDefaultSessionStorePath } from "./paths-C2C4lJH6.js";
import { $ as OPENCLAW_DELIVERY_MIRROR_MODEL, E as persistSessionTranscriptTurn, Z as streamSessionTranscriptLinesReverse, at as redactTranscriptMessage, et as OPENCLAW_TRANSCRIPT_ARTIFACT_API, it as isTranscriptOnlyOpenClawAssistantModel, ot as resolveAndPersistSessionFile, tt as OPENCLAW_TRANSCRIPT_ARTIFACT_PROVIDER } from "./session-accessor-D7yi6P1i.js";
...
async function appendExactAssistantMessageToSessionTranscript(params) {
	...
	if (params.expectedLifecycleRevision && entry?.lifecycleRevision !== params.expectedLifecycleRevision) return {
		ok: false,
		code: "session-rebound",
		reason: `session rebound for sessionKey: ${sessionKey}`
	};
		...
		const turn = await persistSessionTranscriptTurn({
			...
			messages: [{
				...
				shouldAppend: async (target) => {
					latestEquivalentAssistantId = isRedundantDeliveryMirror(params.message) && !identifiedDeliveryMirror ? await findLatestEquivalentAssistantMessageId(target.sessionFile, preparedUnkeyedMessage, params.config) : void 0;
...
function isIdentifiedDeliveryMirror(message) {
	...
	return isRedundantDeliveryMirror(message) && (marker?.kind === "channel-final" || marker?.kind === "channel-final-suppressed");
}
