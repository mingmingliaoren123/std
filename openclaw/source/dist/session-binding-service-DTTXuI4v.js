...
function writePersistedBindings() {
	const records = [...bindingsByConversationKey.values()].filter((record) => !isBindingExpired(record)).toSorted((a, b) => a.bindingId.localeCompare(b.bindingId));
...
function loadBindingsIntoMemory() {
	...
	for (const record of readPersistedBindings()) bindingsByConversationKey.set(buildConversationKey(record.conversation), record);
}
...
async function unbindGenericCurrentConversationBindings(input) {
	...
	if (removed.length > 0) writePersistedBindings();
	return removed;
}
...
//#endregion
export { testing as a, registerSessionBindingAdapter as i, getSessionBindingService as n, unregisterSessionBindingAdapter as o, isSessionBindingError as r, normalizeConversationText as s, SessionBindingError as t };
