...
import { c as tryReadJsonSync, t as JsonFileReadError } from "./json-CI_xQ2Rw.js";
...
import { _ as summarizeDeviceTokens, c as listApprovedPairedDeviceRoles, l as listDevicePairing } from "./device-pairing-Dw7KWdQ7.js";
...
function pairingStoreReadIssueToHealthFinding(error) {
	return {
		checkId: DEVICE_PAIRING_CHECK_ID,
		severity: "warning",
...
async function collectDevicePairingHealthFindings(params) {
	...
	try {
		...
		if (error instanceof JsonFileReadError) return [pairingStoreReadIssueToHealthFinding(error)];
		throw error;
	}
	if (!snapshot) return [];
	return [
