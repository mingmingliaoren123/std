...
import { n as getBridgeAuthForPort } from "./bridge-auth-registry-Bq2jM9YD.js";
import { h as resolveBrowserRateLimitMessage } from "./cdp.helpers-B9jipAGQ.js";
...
async function fetchHttpJson(url, init) {
	...
	try {
		...
		if (!res.ok) {
			...
			throw new BrowserServiceError((body ? new TextDecoder().decode(body) : "") || `HTTP ${res.status}`);
		}
