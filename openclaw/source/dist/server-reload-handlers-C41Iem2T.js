...
import { n as loadInstalledPluginIndexInstallRecordsSync, t as loadInstalledPluginIndexInstallRecords } from "./installed-plugin-index-record-reader-Dd35fnKn.js";
...
import { h as resolveConfigWriteFollowUp } from "./runtime-snapshot-DOvWRYVz.js";
import "./config-DbyjySSE.js";
import { n as isRestartEnabled } from "./commands.flags-CQNnEFjM.js";
import { f as resolveGatewayRestartDeferralTimeoutMs, m as setGatewaySigusr1RestartPolicy, o as deferGatewayRestartUntilIdle, s as emitGatewayRestart } from "./restart-BF68zBs3.js";
import { a as resetModelCatalogCache, n as loadModelCatalog } from "./model-catalog-BfvH9gPq.js";
import { r as clearCurrentProviderAuthState } from "./model-provider-auth-state-DW_JYm-o.js";
import { i as warmCurrentProviderAuthStateOffMainThread } from "./model-provider-auth-0hid3sMh.js";
...
import { a as refreshContextWindowCache } from "./context-_zWLdTOu.js";
import { u as getTotalQueueSize } from "./command-queue-DqzpsN4m.js";
import { n as getInspectableActiveTaskRestartBlockers } from "./task-registry.maintenance-CeBupGdg.js";
import { r as disposeAllSessionMcpRuntimes } from "./agent-bundle-mcp-runtime--G82BMQs.js";
import "./agent-bundle-mcp-tools-CYL4t7pb.js";
import { t as getTotalPendingReplies } from "./dispatcher-registry-CaTZukRA.js";
import { r as resetDirectoryCache } from "./target-resolver-C3zuFudK.js";
import { n as clearSecretsRuntimeSnapshot, o as getActiveSecretsRuntimeSnapshot } from "./runtime-state-CPERULut.js";
import { t as bumpSkillsSnapshotVersion } from "./refresh-state-DHnXO3IV.js";
import { a as diffConfigPaths, n as listPluginInstallTimestampMetadataPaths, r as listPluginInstallWholeRecordPaths, t as buildGatewayReloadPlan } from "./config-reload-plan-BWsbDT0K.js";
import { t as resolveGatewayReloadSettings } from "./config-reload-settings-BUK2a-7x.js";
import { v as resolveHooksConfig } from "./hooks-Bjrm8pWp.js";
import { t as buildGatewayCronService } from "./server-cron-Cwg2hJro.js";
import { n as applyGatewayLaneConcurrency, t as resolveHookClientIpConfig } from "./hook-client-ip-config-u0c3DDHI.js";
import { n as markGatewayModelCatalogStaleForReload } from "./server-model-catalog-DA09KJne.js";
import { t as startGatewayChannelHealthMonitor } from "./server-runtime-startup-services-BI_KXCQu.js";
import { i as startGatewayCronWithLogging } from "./server-runtime-services-BuoTeJhG.js";
...
function createGatewayReloadHandlers(params) {
	...
	const waitForActiveWorkBeforeChannelReload = async (channels, nextConfig) => {
		...
		while (true) {
			if (abortGeneration !== void 0 && myGeneration <= abortGeneration) return true;
	...
	const applyHotReload = async (plan, nextConfig) => {
		...
		if (plan.reloadPlugins) {
			const stopChannelsBeforePluginReplace = async (channels) => {
				...
				if (await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig)) {
					params.logChannels.info("channel reload before plugin replace cancelled by in-process restart");
		...
		if (plan.restartGmailWatcher) {
			...
			try {
				if (!restartAbortController.signal.aborted) {
					const [{ stopGmailWatcher }, { startGmailWatcherWithLogs }] = await Promise.all([import("./gmail-watcher-Dj24WzE-.js"), import("./gmail-watcher-lifecycle-NyjE4tvi.js")]);
		...
		if (channelsToRestart.size > 0) if (shouldSkipChannelRestart()) params.logChannels.info("skipping channel reload (OPENCLAW_SKIP_CHANNELS=1 or OPENCLAW_SKIP_PROVIDERS=1)");
			...
			if (!plan.reloadPlugins && !cancelledByRestart) cancelledByRestart = await waitForActiveWorkBeforeChannelReload(channelsToRestart, nextConfig);
			if (cancelledByRestart) params.logChannels.info("channel restart cancelled by in-process restart");
			else {
				const stopFailures = await collectChannelOperationFailures({
					channels: channelsToRestart,
					run: async (channel) => {
				...
				if (stopFailures.length > 0) throw new Error(`failed to stop channels during suppressed hot reload: ${stopFailures.join(", ")}`);
...
//#endregion
export { createGatewayReloadHandlers as n, startManagedGatewayConfigReloader as r, abortPendingChannelReloads as t };
