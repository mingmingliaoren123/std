...
async function probeDiscord(token, timeoutMs, opts) {
	...
	let res;
	try {
		res = await fetchWithTimeout(`${DISCORD_API_BASE}/users/@me`, { headers: { Authorization: `Bot ${normalized}` } }, timeoutMs, getResolvedFetch(fetcher));
		if (!res.ok) {
		...
		const json = await readProviderJsonResponse(res, "discord.probe.getMe");
		result.ok = true;
		result.bot = {
			id: json.id ?? null,
