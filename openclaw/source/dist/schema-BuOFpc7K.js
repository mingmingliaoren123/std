...
const SkillsCuratorStatusParamsSchema = Type.Object({}, { additionalProperties: false });
const SkillsCuratorStatusResultSchema = Type.Object({
...
const SkillsCuratorActionParamsSchema = Type.Object({ skill: NonEmptyString }, { additionalProperties: false });
...
const AgentsWorkspaceEntrySchema = Type.Object({
	path: NonEmptyString,
	name: NonEmptyString,
...
const AgentsWorkspaceListParamsSchema = Type.Object({
	agentId: NonEmptyString,
	path: Type.Optional(Type.String()),
...
const AgentsWorkspaceListResultSchema = Type.Object({
	agentId: NonEmptyString,
	path: Type.String(),
...
const AgentsWorkspaceFileSchema = Type.Object({
	path: NonEmptyString,
	name: NonEmptyString,
...
const AgentsWorkspaceGetParamsSchema = Type.Object({
	agentId: NonEmptyString,
	path: NonEmptyString
}, { additionalProperties: false });
/** Result for reading one workspace file. */
const AgentsWorkspaceGetResultSchema = Type.Object({
	agentId: NonEmptyString,
...
const AuditListParamsSchema = Type.Object({
	agentId: Type.Optional(NonEmptyString),
	sessionKey: Type.Optional(NonEmptyString),
	runId: Type.Optional(NonEmptyString),
...
const AuditListResultSchema = Type.Object({
...
*/
const TtsSpeakParamsSchema = Type.Object({ text: NonEmptyString }, { additionalProperties: false });
...
const TtsSpeakResultSchema = Type.Object({
	audioBase64: NonEmptyString,
	provider: NonEmptyString,
	outputFormat: Type.Optional(Type.String()),
	mimeType: Type.Optional(Type.String()),
...
*/
const CrestodianChatParamsSchema = Type.Object({
	sessionId: NonEmptyString,
	message: Type.Optional(Type.String()),
...
const CrestodianChatResultSchema = Type.Object({
	sessionId: NonEmptyString,
...
*/
const CrestodianSetupDetectParamsSchema = Type.Object({}, { additionalProperties: false });
...
const CrestodianSetupDetectResultSchema = Type.Object({
	...
	manualProviders: Type.Array(Type.Object({
...
const CrestodianSetupActivateParamsSchema = Type.Object({
	kind: Type.Union([
		Type.Literal("existing-model"),
		Type.Literal("openai-api-key"),
		Type.Literal("anthropic-api-key"),
...
const CrestodianSetupActivateResultSchema = Type.Object({
	ok: Type.Boolean(),
...
const CronDeclarativeAddResultSchema = Type.Object({
...
*/
const DevicePairSetupCodeParamsSchema = Type.Object({
...
*/
const DevicePairSetupCodeResultSchema = Type.Object({
...
const GATEWAY_SERVER_CAPS = { CHAT_SEND_ROUTING_CONTRACT: "chat-send-routing-contract" };
/**
...
const HelloOkSchema = Type.Object({
	...
	controlUiTabs: Type.Optional(Type.Array(Type.Object({
...
const SessionWorktreeInfoSchema = Type.Object({
	id: NonEmptyString,
	path: NonEmptyString,
...
const SystemInfoParamsSchema = Type.Object({}, { additionalProperties: false });
/** Gateway host identity and resource snapshot. */
const SystemInfoResultSchema = Type.Object({
...
const TerminalOpenParamsSchema = Type.Object({
	agentId: Type.Optional(NonEmptyString),
	cols: TerminalDimension,
	rows: TerminalDimension
}, { additionalProperties: false });
/** Result of a successful open; carries the facts the UI header renders. */
const TerminalOpenResultSchema = Type.Object({
	sessionId: NonEmptyString,
	agentId: NonEmptyString,
...
const TerminalInputParamsSchema = Type.Object({
	sessionId: NonEmptyString,
	data: Type.String()
}, { additionalProperties: false });
/** Resizes the PTY grid after the client viewport changes. */
const TerminalResizeParamsSchema = Type.Object({
	sessionId: NonEmptyString,
	cols: TerminalDimension,
	rows: TerminalDimension
}, { additionalProperties: false });
/** Closes a session and kills its process tree. */
const TerminalCloseParamsSchema = Type.Object({ sessionId: NonEmptyString }, { additionalProperties: false });
/**
...
*/
const TerminalAttachParamsSchema = Type.Object({ sessionId: NonEmptyString }, { additionalProperties: false });
/** Result of a successful attach; mirrors open plus the replay buffer. */
const TerminalAttachResultSchema = Type.Object({
	sessionId: NonEmptyString,
	agentId: NonEmptyString,
	shell: NonEmptyString,
	cwd: NonEmptyString,
...
const TerminalSessionInfoSchema = Type.Object({
	sessionId: NonEmptyString,
	agentId: NonEmptyString,
	shell: NonEmptyString,
	cwd: NonEmptyString,
...
*/
const TerminalListResultSchema = Type.Object({ sessions: Type.Array(TerminalSessionInfoSchema) }, { additionalProperties: false });
/** Reads the current output buffer as plain text without attaching. */
const TerminalTextParamsSchema = Type.Object({ sessionId: NonEmptyString }, { additionalProperties: false });
/** Plain-text buffer contents (ANSI stripped); an agent/LLM affordance. */
const TerminalTextResultSchema = Type.Object({ text: Type.String() }, { additionalProperties: false });
/** Shared ok/void result for input, resize, and close. */
const TerminalAckResultSchema = Type.Object({ ok: Type.Boolean() }, { additionalProperties: false });
/** Streamed output chunk; seq lets the client detect gaps and preserve order. */
const TerminalDataEventSchema = Type.Object({
	sessionId: NonEmptyString,
	seq: Type.Integer({ minimum: 0 }),
	data: Type.String()
}, { additionalProperties: false });
/** Terminal end-of-life notice; the session id is invalid after this event. */
const TerminalExitEventSchema = Type.Object({
	sessionId: NonEmptyString,
...
const WorktreesListParamsSchema = Type.Object({}, { additionalProperties: false });
const WorktreesListResultSchema = Type.Object({ worktrees: Type.Array(WorktreeRecordSchema) }, { additionalProperties: false });
const WorktreesCreateParamsSchema = Type.Object({
	repoRoot: NonEmptyString,
	name: Type.Optional(WorktreeNameSchema),
	baseRef: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
const WorktreesRemoveParamsSchema = Type.Object({
	id: NonEmptyString,
	force: Type.Optional(Type.Boolean())
}, { additionalProperties: false });
const WorktreesRemoveResultSchema = Type.Object({
	removed: Type.Boolean(),
	snapshotRef: Type.Optional(NonEmptyString)
}, { additionalProperties: false });
const WorktreesRestoreParamsSchema = Type.Object({ id: NonEmptyString }, { additionalProperties: false });
const WorktreesGcParamsSchema = Type.Object({}, { additionalProperties: false });
const WorktreesGcResultSchema = Type.Object({
...
//#endregion
export { SessionsCompactionGetParamsSchema as $, SkillsProposalsListResultSchema as $i, CronRunParamsSchema as $n, TalkSpeakResultSchema as $r, NodeRenameParamsSchema as $t, TerminalResizeParamsSchema as A, AgentsListResultSchema as Ai, ExecApprovalGetParamsSchema as An, TalkClientCreateParamsSchema as Ar, PluginsUiDescriptorsParamsSchema as At, TasksListResultSchema as B, SkillsDetailResultSchema as Bi, EnvironmentsListParamsSchema as Bn, TalkSessionCancelOutputParamsSchema as Br, NodePairApproveParamsSchema as Bt, TerminalDataEventSchema as C, AgentsFilesGetParamsSchema as Ci, DevicePairApproveParamsSchema as Cn, ChannelsStartParamsSchema as Cr, PushTestResultSchema as Ct, TerminalListResultSchema as D, AgentsFilesSetParamsSchema as Di, DevicePairSetupCodeParamsSchema as Dn, TalkAgentControlResultSchema as Dr, WebPushVapidPublicKeyParamsSchema as Dt, TerminalInputParamsSchema as E, AgentsFilesListResultSchema as Ei, DevicePairRemoveParamsSchema as En, ChannelsStopParamsSchema as Er, WebPushUnsubscribeParamsSchema as Et, TasksCancelParamsSchema as F, SkillsCuratorActionParamsSchema as Fi, ExecApprovalsNodeSetParamsSchema as Fn, TalkConfigParamsSchema as Fr, NodeEventParamsSchema as Ft, SessionFileEntrySchema as G, SkillsProposalInspectParamsSchema as Gi, errorShape as Gn, TalkSessionJoinParamsSchema as Gr, NodePairVerifyParamsSchema as Gt, SystemInfoResultSchema as H, SkillsProposalActionParamsSchema as Hi, EnvironmentsStatusParamsSchema as Hn, TalkSessionCloseParamsSchema as Hr, NodePairRejectParamsSchema as Ht, TasksCancelResultSchema as I, SkillsCuratorActionResultSchema as Ii, ExecApprovalsNodeSnapshotSchema as In, TalkConfigResultSchema as Ir, NodeEventResultSchema as It, SessionWorktreeInfoSchema as J, SkillsProposalRequestRevisionParamsSchema as Ji, CronDeclarativeAddResultSchema as Jn, TalkSessionSteerParamsSchema as Jr, NodePendingDrainResultSchema as Jt, SessionFileKindSchema as K, SkillsProposalInspectResultSchema as Ki, CronAddParamsSchema as Kn, TalkSessionJoinResultSchema as Kr, NodePendingAckParamsSchema as Kt, TasksGetParamsSchema as L, SkillsCuratorStatusParamsSchema as Li, ExecApprovalsSetParamsSchema as Ln, TalkEventSchema as Lr, NodeInvokeParamsSchema as Lt, TerminalTextParamsSchema as M, AgentsUpdateResultSchema as Mi, ExecApprovalResolveParamsSchema as Mn, TalkClientSteerParamsSchema as Mr, PluginApprovalRequestParamsSchema as Mt, TerminalTextResultSchema as N, ModelsListParamsSchema as Ni, ExecApprovalsGetParamsSchema as Nn, TalkClientToolCallParamsSchema as Nr, PluginApprovalResolveParamsSchema as Nt, TerminalOpenParamsSchema as O, AgentsFilesSetResultSchema as Oi, DeviceTokenRevokeParamsSchema as On, TalkCatalogParamsSchema as Or, PluginsSessionActionParamsSchema as Ot, TaskSummarySchema as P, SkillsBinsParamsSchema as Pi, ExecApprovalsNodeGetParamsSchema as Pn, TalkClientToolCallResultSchema as Pr, NodeDescribeParamsSchema as Pt, SessionsCompactionBranchParamsSchema as Q, SkillsProposalsListParamsSchema as Qi, CronRemoveParamsSchema as Qn, TalkSpeakParamsSchema as Qr, NodePresenceAliveReasonSchema as Qt, TasksGetResultSchema as R, SkillsCuratorStatusResultSchema as Ri, EnvironmentStatusSchema as Rn, TalkModeParamsSchema as Rr, NodeInvokeResultParamsSchema as Rt, TerminalCloseParamsSchema as S, WakeParamsSchema as Sa, AgentsFileEntrySchema as Si, StateVersionSchema as Sn, ChannelsLogoutParamsSchema as Sr, PushTestParamsSchema as St, TerminalExitEventSchema as T, AgentsFilesListParamsSchema as Ti, DevicePairRejectParamsSchema as Tn, ChannelsStatusResultSchema as Tr, WebPushTestParamsSchema as Tt, SessionFileBrowserEntrySchema as U, SkillsProposalApplyResultSchema as Ui, EnvironmentsStatusResultSchema as Un, TalkSessionCreateParamsSchema as Ur, NodePairRemoveParamsSchema as Ut, SystemInfoParamsSchema as V, SkillsInstallParamsSchema as Vi, EnvironmentsListResultSchema as Vn, TalkSessionCancelTurnParamsSchema as Vr, NodePairListParamsSchema as Vt, SessionFileBrowserResultSchema as W, SkillsProposalCreateParamsSchema as Wi, ErrorCodes as Wn, TalkSessionCreateResultSchema as Wr, NodePairRequestParamsSchema as Wt, SessionsCleanupParamsSchema as X, SkillsProposalReviseParamsSchema as Xi, CronJobSchema as Xn, TalkSessionTurnParamsSchema as Xr, NodePendingEnqueueResultSchema as Xt, SessionsAbortParamsSchema as Y, SkillsProposalRequestRevisionResultSchema as Yi, CronGetParamsSchema as Yn, TalkSessionSubmitToolResultParamsSchema as Yr, NodePendingEnqueueParamsSchema as Yt, SessionsCompactParamsSchema as Z, SkillsProposalUpdateParamsSchema as Zi, CronListParamsSchema as Zn, TalkSessionTurnResultSchema as Zr, NodePresenceAlivePayloadSchema as Zt, WizardStatusResultSchema as _, AgentParamsSchema as _a, AgentSummarySchema as _i, ResponseFrameSchema as _n, UpdateRunParamsSchema as _r, SessionsResolveParamsSchema as _t, WorktreesGcResultSchema as a, SkillsSkillCardResultSchema as aa, AuditListParamsSchema as ai, ChatMessageGetResultSchema as an, CrestodianSetupActivateParamsSchema as ar, SessionsDescribeParamsSchema as at, TerminalAttachParamsSchema as b, PollParamsSchema as ba, AgentsDeleteParamsSchema as bi, PresenceEntrySchema as bn, CommandsListParamsSchema as br, SecretsResolveParamsSchema as bt, WorktreesRemoveParamsSchema as c, SkillsUploadBeginParamsSchema as ca, ArtifactsDownloadParamsSchema as ci, LogsTailParamsSchema as cn, CrestodianSetupDetectResultSchema as cr, SessionsFilesListParamsSchema as ct, WizardCancelParamsSchema as d, ToolsCatalogParamsSchema as da, AgentsWorkspaceEntrySchema as di, ErrorShapeSchema as dn, ConfigPatchParamsSchema as dr, SessionsMessagesSubscribeParamsSchema as dt, SkillsSearchParamsSchema as ea, TtsSpeakParamsSchema as ei, ChatAbortParamsSchema as en, CronRunsParamsSchema as er, SessionsCompactionListParamsSchema as et, WizardNextParamsSchema as f, ToolsEffectiveParamsSchema as fa, AgentsWorkspaceFileSchema as fi, EventFrameSchema as fn, ConfigSchemaLookupParamsSchema as fr, SessionsMessagesUnsubscribeParamsSchema as ft, WizardStatusParamsSchema as g, AgentIdentityResultSchema as ga, AgentsWorkspaceListResultSchema as gi, RequestFrameSchema as gn, ConfigSetParamsSchema as gr, SessionsResetParamsSchema as gt, WizardStartResultSchema as h, AgentIdentityParamsSchema as ha, AgentsWorkspaceListParamsSchema as hi, HelloOkSchema as hn, ConfigSchemaResponseSchema as hr, SessionsPreviewParamsSchema as ht, WorktreesGcParamsSchema as i, SkillsSkillCardParamsSchema as ia, AuditEventSchema as ii, ChatMessageGetParamsSchema as in, CrestodianChatResultSchema as ir, SessionsDeleteParamsSchema as it, TerminalSessionInfoSchema as j, AgentsUpdateParamsSchema as ji, ExecApprovalRequestParamsSchema as jn, TalkClientCreateResultSchema as jr, PluginsUiDescriptorsResultSchema as jt, TerminalOpenResultSchema as k, AgentsListParamsSchema as ki, DeviceTokenRotateParamsSchema as kn, TalkCatalogResultSchema as kr, PluginsSessionActionResultSchema as kt, WorktreesRemoveResultSchema as l, SkillsUploadChunkParamsSchema as la, ArtifactsGetParamsSchema as li, LogsTailResultSchema as ln, ConfigApplyParamsSchema as lr, SessionsFilesListResultSchema as lt, WizardStartParamsSchema as m, AgentEventSchema as ma, AgentsWorkspaceGetResultSchema as mi, GatewayFrameSchema as mn, ConfigSchemaParamsSchema as mr, SessionsPluginPatchParamsSchema as mt, WorktreeRecordSchema as n, SkillsSecurityVerdictsParamsSchema as na, WebLoginStartParamsSchema as ni, ChatHistoryParamsSchema as nn, CronUpdateParamsSchema as nr, SessionsCreateParamsSchema as nt, WorktreesListParamsSchema as o, SkillsStatusParamsSchema as oa, AuditListResultSchema as oi, ChatMetadataParamsSchema as on, CrestodianSetupActivateResultSchema as or, SessionsFilesGetParamsSchema as ot, WizardNextResultSchema as p, ToolsInvokeParamsSchema as pa, AgentsWorkspaceGetParamsSchema as pi, GATEWAY_SERVER_CAPS as pn, ConfigSchemaLookupResultSchema as pr, SessionsPatchParamsSchema as pt, SessionFileRelevanceSchema as q, SkillsProposalRecordResultSchema as qi, CronAddResultSchema as qn, TalkSessionOkResultSchema as qr, NodePendingDrainParamsSchema as qt, WorktreesCreateParamsSchema as r, SkillsSecurityVerdictsResultSchema as ra, WebLoginWaitParamsSchema as ri, ChatInjectParamsSchema as rn, CrestodianChatParamsSchema as rr, SessionsCreateResultSchema as rt, WorktreesListResultSchema as s, SkillsUpdateParamsSchema as sa, ArtifactSummarySchema as si, ChatSendParamsSchema as sn, CrestodianSetupDetectParamsSchema as sr, SessionsFilesGetResultSchema as st, ProtocolSchemas as t, SkillsSearchResultSchema as ta, TtsSpeakResultSchema as ti, ChatEventSchema as tn, CronStatusParamsSchema as tr, SessionsCompactionRestoreParamsSchema as tt, WorktreesRestoreParamsSchema as u, SkillsUploadCommitParamsSchema as ua, ArtifactsListParamsSchema as ui, ConnectParamsSchema as un, ConfigGetParamsSchema as ur, SessionsListParamsSchema as ut, WizardStepSchema as v, AgentWaitParamsSchema as va, AgentsCreateParamsSchema as vi, ShutdownEventSchema as vn, UpdateStatusParamsSchema as vr, SessionsSendParamsSchema as vt, TerminalEventSchema as w, AgentsFilesGetResultSchema as wi, DevicePairListParamsSchema as wn, ChannelsStatusParamsSchema as wr, WebPushSubscribeParamsSchema as wt, TerminalAttachResultSchema as x, SendParamsSchema as xa, AgentsDeleteResultSchema as xi, SnapshotSchema as xn, CommandsListResultSchema as xr, SecretsResolveResultSchema as xt, TerminalAckResultSchema as y, MessageActionParamsSchema as ya, AgentsCreateResultSchema as yi, TickEventSchema as yn, COMMAND_DESCRIPTION_MAX_LENGTH as yr, SessionsUsageParamsSchema as yt, TasksListParamsSchema as z, SkillsDetailParamsSchema as zi, EnvironmentSummarySchema as zn, TalkSessionAppendAudioParamsSchema as zr, NodeListParamsSchema as zt };
