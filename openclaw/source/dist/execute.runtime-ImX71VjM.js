...
import { s as sanitizeHostExecEnv } from "./host-env-security-CWC2ZCy4.js";
...
import { c as emitAgentEvent, t as assertAgentRunLifecycleGenerationCurrent } from "./agent-events-CRggPZCM.js";
import { i as classifyFailoverReason } from "./errors-XbAR6hS3.js";
import { d as resolveFailoverStatus, t as FailoverError } from "./failover-error-B2zFYEnG.js";
...
import { n as applySkillEnvOverridesFromSnapshot } from "./env-overrides-K88qrVfF.js";
...
import { a as isMessagingToolSendAction, i as isMessagingToolDeliveryAction, o as isMessagingToolTargetEvidenceAction, r as isMessagingTool } from "./embedded-agent-messaging-AJZX3UxO.js";
import { a as extractMessagingToolSendResult, h as sanitizeToolResult, i as extractMessagingToolSend, m as sanitizeToolArgs, n as collectMessagingMediaUrlsFromRecord, o as extractMessagingToolSourceReplyPayload, r as collectMessagingMediaUrlsFromToolResult } from "./embedded-agent-subscribe.tools-CfJXhZwg.js";
...
import { _ as waitForMcpLoopbackToolCallCaptureIdle, r as clearMcpLoopbackToolCallCapture, t as beginMcpLoopbackToolCallCapture } from "./mcp-http.loopback-runtime-DGZ9iwzQ.js";
import { a as shouldUseClaudeLiveSession, c as parseCliOutput, i as runClaudeLiveSessionTurn, o as createCliJsonlStreamingParser, r as rotateClaudeLiveMcpCaptureKeyForContext, s as extractCliErrorMessage } from "./claude-live-session-CgMGQ6on.js";
import { c as resolvePromptInput, d as writeCliSystemPromptFile, f as buildCliSupervisorScopeKey, h as prepareCliBundleMcpCaptureAttempt, i as enqueueCliRun, l as resolveSessionIdToSend, m as resolveCliRunTimeoutOverrideMs, o as prepareCliPromptImagePayload, p as resolveCliNoOutputTimeoutMs, r as buildCliArgs, s as resolveCliRunQueueKey, t as buildClaudeOwnerKey, u as resolveSystemPromptUsage } from "./helpers-Greagbnf.js";
...
async function executePreparedCliRun(context, cliSessionIdToUse) {
	...
	const systemPromptFile = systemPromptArg && (!useResume || backend.systemPromptWhen === "always" || resendSystemPromptForSoftResume) ? await executeDeps.writeCliSystemPromptFile({
		backend,
	...
	try {
		completedOutput = await enqueueCliRun(queueKey, async () => {
			...
			const resolveToolTerminalReason = (error) => {
				...
				return error instanceof FailoverError && error.reason === "timeout" ? "timed_out" : "failed";
			};
			...
			try {
				...
				const beginGatewayCapture = (captureKey) => {
					...
					beginMcpLoopbackToolCallCapture({
						...
						onToolCallUpdate: ({ previous, current }) => {
							...
							if (candidates.length === 1 && !candidates[0]?.ambiguous) candidates[0].current = current;
				...
				const emitParsedToolTerminal = (event) => {
					...
					const now = Date.now();
					const terminalReason = (trustedOutcome && trustedOutcome.outcome !== "blocked" && trustedOutcome.outcome !== "completed" && trustedOutcome.outcome !== "unknown" ? trustedOutcome.outcome : void 0) ?? resolveToolTerminalReason(event.incomplete ? runError : void 0);
				...
				if (shouldUseClaudeLiveSession(context)) {
					...
					if (parsed.errorText) {
						const reason = classifyFailoverReason(parsed.errorText, { provider: params.provider }) ?? "unknown";
