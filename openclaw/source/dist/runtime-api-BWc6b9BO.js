...
import { t as createWorkboardSqliteStores } from "./sqlite-store-guctwyfg.js";
...
async function materializeWorkspace(params) {
	...
	if (!params.allowManagedWorktrees) throw new Error("managed worktree dispatch requires operator.admin");
...
async function dispatchAndStartWorkboardCards(params) {
	...
	for (const card of selectStartableCards(cards, maxStarts, candidates)) {
		...
		if (card.metadata?.automation?.workspace?.kind === "worktree" && params.options?.allowManagedWorktrees === false) {
			startFailures.push({
				cardId: card.id,
				title: card.title,
		...
		try {
			...
			if (materializedWorkspace) await params.store.update(card.id, { workspace: materializedWorkspace });
			...
			if (!runStarted && materializedWorkspace?.path && params.worktrees) {
				await params.worktrees.removeIfLossless({ path: materializedWorkspace.path }).catch(() => void 0);
...
async function cleanupWorkboardRunWorktree(params) {
	const workspace = (await params.store.list()).find((entry) => entry.runId === params.runId)?.metadata?.automation?.workspace;
	if (workspace?.kind !== "worktree" || !workspace.path) return;
...
function registerWorkboardGatewayMethods(params) {
	...
	api.registerGatewayMethod("workboard.cards.dispatch", async ({ params: requestParams, respond, client }) => {
		try {
			...
			const result = await dispatchAndStartWorkboardCards({
				...
				options: {
					...
					allowManagedWorktrees: Array.isArray(client?.connect?.scopes) && client.connect.scopes.includes("operator.admin")
				}
			});
			respond(true, {
				...result,
...
//#endregion
export { WorkboardStore as i, cleanupWorkboardRunWorktree as n, dispatchAndStartWorkboardCards as r, registerWorkboardGatewayMethods as t };
