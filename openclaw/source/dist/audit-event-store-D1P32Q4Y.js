...
function pruneAuditEventsAfterInsert(db, now) {
	...
	executeSqliteQuerySync(db, kysely.deleteFrom("audit_events").where("occurred_at", "<", now - AUDIT_EVENT_RETENTION_MS));
	const overflowRow = executeSqliteQueryTakeFirstSync(db, kysely.selectFrom("audit_events").select("sequence").orderBy("sequence", "desc").offset(AUDIT_EVENT_MAX_ROWS).limit(1));
	const sequenceCutoff = overflowRow ? normalizeSqliteNumber(overflowRow.sequence) : void 0;
	if (sequenceCutoff !== void 0) executeSqliteQuerySync(db, kysely.deleteFrom("audit_events").where("sequence", "<=", sequenceCutoff));
}
/** Persist one projected event idempotently and prune fixed retention bounds. */
function recordAuditEvent(input, options = {}) {
	return runOpenClawStateWriteTransaction(({ db }) => {
		const insert = executeSqliteQuerySync(db, getAuditKysely(db).insertInto("audit_events").values(bindAuditEvent(input)).onConflict((conflict) => conflict.column("source_id").doNothing()));
		const insertedSequence = insert.insertId ? Number(insert.insertId) : void 0;
		if (insertedSequence === void 0) return;
