...
import { _ as executeSqliteQueryTakeFirstSync, g as executeSqliteQuerySync, i as openOpenClawStateDatabase, l as resolveOpenClawStateSqlitePath, m as requireNodeSqlite, o as runOpenClawStateWriteTransaction, v as getNodeSqliteKysely } from "./openclaw-state-db-DzSsA9Ji.js";
import { i as resolveAgentModelPrimaryValue } from "./model-input-B5RmygIK.js";
import { At as boolean } from "./schemas-CBJjibl3.js";
import { V as MEMORY_DREAMING_SYSTEM_EVENT_TEXT } from "./dreaming-DqzMSXGC.js";
import { a as loadCronQuarantineFile, c as resolveCronQuarantinePath, d as saveCronJobsStoreWithMetadata, f as saveCronQuarantineFile, h as getInvalidPersistedCronJobReason, i as loadCronJobsStoreWithConfigJobsReadOnly, r as loadCronJobsStoreWithConfigJobs, s as resolveCronJobsStorePath, u as saveCronJobsStore } from "./store-ScQ9SjOe.js";
import { a as LowercaseNonEmptyStringFieldSchema, i as DeliveryThreadIdFieldSchema, n as normalizeCronJobInput, o as TrimmedNonEmptyStringFieldSchema, s as parseOptionalField } from "./normalize-BMkddmz2.js";
import { i as parseAbsoluteTimeMs, r as resolveDefaultCronStaggerMs, t as normalizeCronStaggerMs } from "./stagger-q3jv0Ns2.js";
import { s as inferCronJobName, u as coerceFiniteScheduleNumber } from "./session-target-YpyFax1g.js";
import { c as parseCronRunLogEntryObject, i as readCronRunLogEntriesPage, t as appendCronRunLog } from "./run-log-DIhrTrSU.js";
...
async function sha256File(filePath) {
	return createHash("sha256").update(await fs$1.readFile(filePath)).digest("hex");
}
...
async function restoreArchivedSource(archivePath, sourcePath, expectedSha256) {
	...
	try {
		await fs$1.rename(archivePath, sourcePath);
	} catch (err) {
		if (err.code === "EXDEV") {
...
async function copyLegacyCronFileAcrossDevices(filePath, initialArchivePath, expectedSha256, useNumberedArchive = true) {
	...
	try {
		const sourceStat = await fs$1.stat(filePath);
		...
		for (let index = 2;; index += 1) try {
			const archiveHandle = await fs$1.open(archivePath, "wx", sourceMode | 384);
			archiveCreated = true;
		...
		await fs$1.copyFile(filePath, archivePath);
		...
		await fs$1.chmod(archivePath, sourceMode | 384);
		const archiveHandle = await fs$1.open(archivePath, "r+");
		try {
		...
		const currentSourceStat = await fs$1.stat(filePath);
...
async function archiveLegacyCronFile(filePath, expectedSha256) {
	...
	try {
		await fs$1.rename(filePath, archivePath);
	} catch (err) {
...
//#endregion
export { collectLegacyCronStoreHealthFindings, collectLegacyWhatsAppCrontabHealthWarning, maybeRepairLegacyCronStore, noteLegacyWhatsAppCrontabHealthCheck, repairLegacyCronStoreWithoutPrompt };
