import { n as splitGraphemes, o as truncateToVisibleWidth, s as visibleWidth } from "./ansi-D1GK_odF.js";
...
function replaceHomePath(input, display) {
	...
	while (cursor < input.length) {
		...
		while (punctuationEnd < input.length && /[)"'`:,;.\]}]/u.test(input[punctuationEnd])) punctuationEnd += 1;
		const punctuationEndsToken = punctuationEnd > homeEnd && (punctuationEnd === input.length || /\s/u.test(input[punctuationEnd]));
