import { n as detectErrorKind } from "./errors-sMD712F3.js";
...
import { n as logError } from "./logger-D7QYAmug.js";
...
import { l as loadGatewaySessionRow, u as loadSessionEntry } from "./session-utils-DD3pe_2A.js";
import { d as stripHeartbeatToken } from "./heartbeat-Butmf3AR.js";
import { a as resolveToolSearchCodeDisplayTarget } from "./tool-display-common-CLb0rjKV.js";
...
import { a as createChatRunState, c as createToolEventRecipientRegistry, i as createChatRunRegistry, l as isChatAbortMarkerCurrent, n as createChatAbortMarker, o as createSessionEventSubscriberRegistry, s as createSessionMessageSubscriberRegistry } from "./server-chat-state-_DvSGu8D.js";
import { t as formatForLog } from "./ws-log-DlOPjxAD.js";
import { i as persistGatewaySessionLifecycleEvent, n as isRestartRecoveryLifecycleEvent, r as isStaleLifecycleEventForSession, t as deriveGatewaySessionLifecycleProjectionPatch } from "./session-lifecycle-state-Czgc8l0p.js";
...
function createAgentEventHandler({ broadcast, broadcastToConnIds, nodeSendToSession, agentRunSeq, chatRunState, resolveSessionKeyForRun, clearAgentRunContext, toolEventRecipients, sessionEventSubscribers, sessionMessageSubscribers, loadGatewaySessionRowForSnapshot = loadGatewaySessionRow, lifecycleErrorRetryGraceMs = AGENT_LIFECYCLE_ERROR_RETRY_GRACE_MS, isChatSendRunActive = () => false, clearTrackedActiveRun, markTrackedRunTerminalPersisted, trackTrackedRunTerminalPersistence, resolveActiveLifecycleGenerationForRun = () => void 0, updateRunToolErrorSummary, resolveSessionActiveRunState }) {
	...
	const finalizeLifecycleEvent = (evt, opts) => {
		...
		if (!suppressRestartRecoveryProjection && sessionKey && (isControlUiVisible || (deliverySessionKey ? sessionMessageSubscribers.get(deliverySessionKey).size > 0 : false))) if (!isAborted) {
			...
			if (!(opts?.skipChatErrorFinal && terminalState === "error")) emitChatTerminal(terminalSessionKey, terminalRunId, evt.runId, evt.seq, terminalState, terminalOutcome.error ?? evt.data?.error, terminalOutcome.stopReason, readChatErrorKind(evt.data?.errorKind) ?? detectErrorKind(evt.data?.error), {
