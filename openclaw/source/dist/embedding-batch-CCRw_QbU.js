...
import { g as readResponseTextLimited, h as readProviderTextResponse, m as readProviderJsonResponse } from "./provider-http-errors-HGLTiqMh.js";
import "./string-coerce-runtime-ZbuYDJgZ.js";
import "./provider-http-CwvZqS_e.js";
import { C as withRemoteHttpResponse, T as formatUnavailableBatchError, _ as buildBatchHeaders, b as applyEmbeddingBatchOutputLine, d as uploadBatchJsonlFile, f as resolveBatchCompletionFromStatus, g as runEmbeddingBatchGroups, h as buildEmbeddingBatchGroupOptions, m as throwIfBatchTerminalFailure, p as resolveCompletedBatchResult, v as normalizeBatchBaseUrl, w as extractBatchErrorMessage, x as postJsonWithRetry, y as EMBEDDING_BATCH_ENDPOINT } from "./memory-core-host-engine-embeddings-DVluEqwT.js";
...
async function readOpenAiBatchOutputLines(response, params) {
	let lineCount = 0;
	const emitOutputLine = (line) => {
		lineCount += 1;
		if (lineCount > params.maxLines) throw new Error(`openai.batch-file-content: JSONL output exceeds ${params.maxLines} records`);
		return params.onLine(line);
	};
	const emitParsedLine = (line) => emitOutputLine(parseOpenAiBatchOutputLine(line));
	const reader = response.body?.getReader();
	if (!reader) {
		const text = await readProviderTextResponse(response, "openai.batch-file-content", { maxBytes: OPENAI_BATCH_OUTPUT_LINE_MAX_BYTES });
	...
	const appendSegment = (segment) => {
		...
		if (lineBytes > OPENAI_BATCH_OUTPUT_LINE_MAX_BYTES) throw new Error(`openai.batch-file-content: JSONL line exceeds ${OPENAI_BATCH_OUTPUT_LINE_MAX_BYTES} bytes`);
		line += segment;
	};
	const emitLine = () => {
		...
		line = "";
		lineBytes = 0;
		if (trimmed) return emitParsedLine(trimmed);
		return true;
	};
	...
	try {
		...
		if (!consumeText(decoder.decode())) return;
		if (line.trim()) emitLine();
	} catch (error) {
		await reader.cancel().catch(() => {});
		throw error;
	} finally {
...
async function readOpenAiBatchOutputFile(params) {
	return await fetchOpenAiBatchResource({
		...
		path: `/files/${params.fileId}/content`,
		errorPrefix: "openai batch file content",
		parse: async (res) => await readOpenAiBatchOutputLines(res, {
