...
import { a as formatUncaughtError, c as readErrorName } from "./errors-sMD712F3.js";
...
import { n as emitDiagnosticEvent } from "./diagnostic-events-JZsXee1S.js";
import "./agent-scope-B2Pk_xhT.js";
import { c as parseAgentSessionKey, d as parseThreadSessionSuffix, l as parseRawSessionConversationRef, n as isAcpSessionKey } from "./session-key-utils-A-JGvyXu.js";
import { p as resolveAgentIdFromSessionKey, s as classifySessionKeyShape, u as normalizeAgentId } from "./session-key-VWT_xzM9.js";
...
import { i as resolveAssistantAvatarUrl } from "./control-ui-shared-BxIfxICx.js";
import { d as getAgentEventLifecycleGeneration, h as onAgentEvent, i as clearAgentRunContext, r as claimAgentRunContext, t as assertAgentRunLifecycleGenerationCurrent } from "./agent-events-CRggPZCM.js";
import { i as resolveExplicitAgentSessionKey, r as resolveAgentMainSessionKey } from "./main-session-D7Jmp9DO.js";
import { X as resolveMaintenanceConfigFromInput, lt as resolveSessionStoreKey, st as canonicalizeSpawnedByForAgent, v as updateSessionStore } from "./store-BJJhlPrk.js";
import { i as mergeDeliveryContext, o as normalizeDeliveryContext, s as normalizeSessionDeliveryFields } from "./delivery-context.shared-3o3tBaCD.js";
import { n as isInternalNonDeliveryChannel, t as INTERNAL_MESSAGE_CHANNEL } from "./message-channel-constants-BXOA4cxJ.js";
...
import { a as mergeAgentRunTerminalOutcome, n as buildAgentRunTerminalOutcome, o as normalizeAgentRunTimeoutPhase, t as AGENT_RUN_TERMINAL_RETRY_GRACE_MS } from "./agent-run-terminal-outcome-Dv8Iorx2.js";
...
import { i as createRunningTaskRun, o as finalizeTaskRunByRunId } from "./detached-task-runtime-B93mVzDV.js";
import { n as resolveAgentTimeoutMs } from "./timeout-0Cw4kcol.js";
import { r as readAcpSessionMeta } from "./session-meta-e1OOAdtW.js";
import { b as retainEmbeddedAgentRunAbortabilityForRunId, c as isEmbeddedAgentRunAbortableForRunId, i as clearEmbeddedAgentRunAbortabilityForRunId } from "./runs-B0SQhu92.js";
import "./sessions-oDygYVdy.js";
import { a as hasTerminalMainSessionTranscriptNewerThanRegistrySync, c as resolveSessionWorkStartError, l as resolveTerminalMainSessionTranscriptRegistryCheck, s as resolveSessionLifecycleTimestamps } from "./lifecycle-BS_t5emX.js";
import { c as resolveSessionResetPolicy, n as resolveChannelResetConfig, o as evaluateSessionFreshness, r as resolveSessionResetType } from "./reset-Cmc2g-h4.js";
import { c as shouldPreserveUserFacingSessionStateForInputProvenance, n as annotateInterSessionPromptText, s as normalizeInputProvenance } from "./input-provenance-BkJA7lbx.js";
import { d as migrateAndPruneGatewaySessionStoreKey, g as resolveGatewaySessionStoreTarget, h as resolveGatewayModelSupportsImages, m as resolveFreshestSessionEntryFromStoreKeys, p as resolveDeletedAgentIdFromSessionKey, u as loadSessionEntry, x as resolveSessionModelRef } from "./session-utils-DD3pe_2A.js";
import { n as resolveSendPolicy } from "./send-policy-BIDnzfWE.js";
import { s as resolveTrustedGroupId } from "./agent-tools.policy-YD9HuYgO.js";
import "./subagent-announce-origin-DPSebzOW.js";
import { n as shouldDowngradeDeliveryToSessionOnly } from "./best-effort-delivery-BNjgBWxw.js";
...
import { i as validateAgentWaitParams, n as validateAgentIdentityParams, r as validateAgentParams, t as formatValidationErrors } from "./src-CToKmqGn.js";
import { Gn as errorShape, Wn as ErrorCodes } from "./schema-BuOFpc7K.js";
import { n as normalizeSpawnedRunMetadata, r as resolveIngressWorkspaceOverrideForSpawnedRun } from "./spawned-context-QGy9i1y8.js";
import { n as resolvePublicAgentAvatarSource, t as resolveAgentAvatar } from "./identity-avatar-bf3YaKUg.js";
import { n as beginSessionWorkAdmission } from "./session-lifecycle-admission-DfdITEs1.js";
import { f as updateChatRunProvider, l as resolveAgentRunExpiresAtMs, s as registerChatAbortController } from "./chat-abort-CV5hBwE-.js";
import { a as emitGatewaySessionEndPluginHook, c as performGatewaySessionReset, o as emitGatewaySessionStartPluginHook } from "./session-reset-service-CV7slv6G.js";
import { n as agentCommandFromIngress } from "./agent-command-ABV9I5el.js";
import { i as resolveAgentOutboundTarget, n as resolveAgentDeliveryPlanWithSessionRoute, r as resolveAgentExplicitRecipientSession } from "./agent-delivery-BE_Ki_cN.js";
import { t as formatForLog } from "./ws-log-DlOPjxAD.js";
import "./agent-BheXW3pw.js";
...
import { i as parseMessageWithAttachments, n as MediaOffloadError, o as resolveChatAttachmentMaxBytes, t as normalizeRpcAttachmentsToChatAttachments } from "./attachment-normalize-7nXLb1Jk.js";
import { i as waitForTerminalGatewayDedupe, n as readTerminalSnapshotFromGatewayDedupe, r as setGatewayDedupeEntry, t as emitSessionsChanged } from "./session-change-event-gGmAI0Up.js";
import { r as resolveVoiceWakeRouteByTrigger, t as loadVoiceWakeRoutingConfig } from "./voicewake-routing-D1sSmmLl.js";
...
async function waitForAgentJob(params) {
	...
	return await new Promise((resolve) => {
		...
		const scheduleErrorFinish = (snapshot, delayMs = AGENT_RUN_TERMINAL_RETRY_GRACE_MS) => {
			scheduleTerminalFinish("error", snapshot, delayMs);
		};
		const scheduleTimeoutFinish = (snapshot, delayMs = AGENT_RUN_TERMINAL_RETRY_GRACE_MS) => {
		...
		const timer = setSafeTimeout(() => {
			const pendingErrorCandidate = ignoreCachedSnapshot ? pendingErrorSnapshot : getPendingAgentRunError(runId)?.snapshot;
			if (pendingErrorCandidate) {
				finish(createPendingErrorTimeoutSnapshot(pendingErrorCandidate));
				return;
			}
			const pendingTimeoutCandidate = ignoreCachedSnapshot ? pendingTimeoutSnapshot : getPendingAgentRunTimeout(runId)?.snapshot;
			if (pendingTimeoutCandidate && terminalOutcomeFromSnapshot(pendingTimeoutCandidate)?.reason === "hard_timeout") {
...
async function registerPluginSubagentRunFromGateway(params) {
	...
	const { registerSubagentRun } = await import("./subagent-registry-B93EmgUG.js");
	registerSubagentRun({
		runId: params.runId,
		childSessionKey,
...
const agentHandlers = {
	agent: async ({ params, respond, context, client, isWebchatConnect }) => {
		...
		if (request.cwd && !path.isAbsolute(request.cwd)) {
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, "cwd must be absolute"));
			return;
		}
		if (request.cwd && !normalizeOptionalString(client?.internal?.pluginRuntimeOwnerId)) {
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, "cwd is reserved for plugin-owned subagent runs"));
			return;
		}
		...
		const clearUnacceptedAgentDedupe = () => {
			...
			if (reservedEntry?.ok && isAcceptedAgentDedupePayload(reservedEntry.payload) && reservedEntry.payload.reservationId !== agentReservationId) return;
		...
		if (explicitRecipient) {
			reservePreAcceptedAgentDedupe(void 0, explicitRecipient.agentId);
			try {
		...
		if (explicitRecipientSession?.error) {
			clearUnacceptedAgentDedupe();
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, explicitRecipientSession.error.message));
			return;
		}
		let requestedSessionKey = requestedSessionKeyRaw ?? explicitRecipientSession?.sessionKey ?? (!requestedSessionId ? resolveExplicitAgentSessionKey({
			cfg,
			agentId
		}) : void 0);
		if (agentId && requestedSessionKeyRaw) {
		...
		if (preAcceptedReservedSessionKey) reservePreAcceptedAgentDedupe(preAcceptedReservedSessionKey, agentId);
		...
		try {
			...
			const admissionAgentId = () => resolvedSessionKey === "global" ? resolvedSessionAgentId ?? agentId ?? resolveDefaultAgentId(cfgForAgent ?? cfg) : void 0;
			const assertGatewayWorkAdmissionAllowed = () => {
				...
				if (agentDedupeReserved) {
					...
					if (!latestPreRegistrationAbort.ok || !isAcceptedAgentDedupePayload(latestPreRegistrationAbort.payload)) {
						postAdmissionAbort = latestPreRegistrationAbort;
						return;
					}
					...
					if (!isFutureDateTimestampMs(latestPreRegistrationAbort.payload.expiresAtMs, { nowMs: Date.now() })) {
						postAdmissionTimeout = {
							runId,
							status: "timeout",
							summary: "aborted",
				...
				if (lifecycleRotatedDuringAdmission || !resolvedSessionKey) return;
				let latestEntry = loadSessionEntry(resolvedSessionKey, {
					...resolvedSessionKey === "global" ? { agentId: admissionAgentId() } : {},
					clone: false
				}).entry;
				if (!latestEntry && requestedSessionKey && requestedSessionKey !== resolvedSessionKey) latestEntry = loadSessionEntry(requestedSessionKey, { clone: false }).entry;
				if (sessionPersistedBeforeGatewayAdmission && !latestEntry) throw new Error(`Session "${resolvedSessionKey}" was deleted while starting work. Retry.`);
				...
				if (latestEntry?.sessionId && latestEntry.sessionId !== supersededSessionId) admittedSessionId = latestEntry.sessionId;
			};
			const interruptGatewayWorkAdmission = () => {
				...
				if (reservedEntry?.ok && isAcceptedAgentDedupePayload(reservedEntry.payload) && reservedEntry.payload.reservationId === agentReservationId) setAbortedAgentDedupeEntries({
					dedupe: context.dedupe,
					keys: agentDedupeKeys,
					agentId: admissionAgentId(),
					sessionKey: resolvedSessionKey,
			...
			const acquireGatewayWorkAdmission = async (scope) => {
				if (gatewayWorkAdmission) return;
			...
			const respondToGatewayAdmissionOutcome = () => {
				...
				if (postAdmissionTimeout) {
					gatewayWorkAdmission?.release();
					agentRunAccepted = true;
					respond(true, postAdmissionTimeout, void 0, {
						cached: true,
						runId
					});
					return true;
			...
			if (requestedSessionKey) {
				...
				const { cfg: cfgLocal, storePath, entry, canonicalKey, legacyKey } = loadSessionEntry(requestedSessionKey, sessionLoadOptions);
				cfgForAgent = cfgLocal;
				const sessionExistedBeforeAttachmentSetup = preAttachmentSession?.canonicalKey === canonicalKey ? preAttachmentSession : void 0;
				if (sessionExistedBeforeAttachmentSetup && !entry) {
					respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, `Session "${canonicalKey}" was deleted while starting work. Retry.`));
					return;
				}
				if (sessionExistedBeforeAttachmentSetup && entry?.sessionId !== sessionExistedBeforeAttachmentSetup.sessionId) {
					respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, `Session "${canonicalKey}" changed while starting work. Retry.`));
					return;
				}
				...
				let freshness = entry ? skipImplicitExpiry ? { fresh: true } : evaluateSessionFreshness({
					updatedAt: entry.updatedAt,
					...lifecycleTimestamps,
					now,
					policy: resetPolicy
				}) : void 0;
				const visibleRequest = request.bootstrapContextRunKind !== "cron" && request.bootstrapContextRunKind !== "heartbeat" && !request.internalEvents?.length;
				...
				const buildSessionPatch = (freshEntry) => {
					...
					const freshSkipImplicitExpiry = resetPolicy.configured !== true && hasProviderOwnedSession(freshEntry);
					const freshFreshness = freshEntry ? freshSkipImplicitExpiry ? { fresh: true } : evaluateSessionFreshness({
					...
					const shouldClearTerminalState = freshCanReuseSession && visibleRequest && isRecoverableTerminalSessionStatus(freshEntry?.status) && !freshSessionRotatedSinceLoad && patchSessionId === freshEntry?.sessionId;
					const patch = {
				...
				if (storePath && !suppressVisibleSessionEffects) {
					...
					try {
						persisted = await updateSessionStore(storePath, (store) => {
							...
							const preMigrationEntry = resolveFreshestSessionEntryFromStoreKeys(store, preMigrationTarget.storeKeys);
							const initialEntry = resolveFreshestSessionEntryFromStoreKeys(store, resolveGatewaySessionStoreTarget({
								cfg: cfgLocal,
							...
							const archivedError = resolveSessionWorkStartError(preMigrationTarget.canonicalKey, preMigrationEntry);
							if (archivedError) {
						...
						if (archivedDuringStoreUpdateError) {
							respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, archivedDuringStoreUpdateError));
							return;
						}
					...
					if (patchBuild.isNewSession && entry?.sessionId && resolvedSessionId !== entry.sessionId) supersededSessionId = entry.sessionId;
					admittedSessionId = resolvedSessionId ?? runId;
					try {
				...
				if (isNewSession && entry?.sessionId && resolvedSessionId !== entry.sessionId) supersededSessionId = entry.sessionId;
			...
			const lifecycleStorePath = resolvedSessionKey ? loadSessionEntry(resolvedSessionKey, {
				...resolvedSessionKey === "global" ? { agentId: activeSessionAgentId } : {},
				clone: false
			}).storePath : `agent:${activeSessionAgentId}`;
			try {
				await acquireGatewayWorkAdmission(lifecycleStorePath);
				assertGatewayWorkAdmissionAllowed();
				if (!Boolean(postAdmissionAbort || postAdmissionTimeout || postAdmissionSuperseded || lifecycleRotatedDuringAdmission)) admittedRunAbort = registerChatAbortController({
			...
			if (!activeGatewayWorkAdmission) {
				respond(false, void 0, errorShape(ErrorCodes.UNAVAILABLE, "agent run admission failed"));
				return;
			}
			...
			activeGatewayWorkAdmission.run(async () => {
				...
				try {
					...
					if (!dispatched) cleanupAdmittedRun({ force: true });
				}
			});
		} finally {
			if (!gatewayAdmissionTransferred) gatewayWorkAdmission?.release();
			clearUnacceptedAgentDedupe();
		}
	},
