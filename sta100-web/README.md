...
空数据库首次启动只写入系统默认配置、内置任务和通道默认项，不再写入客户、报价、订单、产品、供应商、私有文件等业务演示记录。客户原始知识数据、OEM 工厂库、联网/RAG 来源、OCR、正式单据模板和离线升级包规范尚未提供，对应接口返回明确 `501 TODO_*`，不会伪造成功。
...
- 单据下载按类型提供格式选择：PI 使用 PDF；CI、PL、合同支持 PDF、Word（`.doc`，可由 Word/WPS 打开）和 Excel（保留内置 `.xlsm` 标准底板）；报关单支持 PDF 和 XML。所有输出都复用同一份客户、订单、产品明细和模板默认值，后端会拒绝不适用的格式。
...
客户私有文件和随包共有文件会在本机解析、分块并建立确定性的本地混合向量索引；当前支持文本、Office 文档和结构化文件，PDF 先保存并标记待处理。联网检索仍须在客户确认站点白名单、授权和留存规则后接入工具层。
